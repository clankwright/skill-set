...
# ladder from CURSOR_GROK_FAMILY in bin/skill-chain.py — bump that constant
# to change the default + ladder):
#   {CURSOR_GROK_FAMILY}-{low,medium,high}[-fast]
# Other Cursor-exposed ids (claude-4.5-sonnet, gpt-5.1, …) also work; bare "grok" does not.
CURSOR_MODEL=<cursor-grok-id> ./bin/skill-chain.py --harness cursor my-cycle
