...
- `bin/install-skills.sh` — copies every `skills/**/<name>/` into the harness's user-skills directory (default `~/.claude/skills/skill-set/<name>/`, flat under the `skill-set/` segregation dir — categories are dropped on install) so the harness picks them up globally.
