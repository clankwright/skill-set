...
The manager NEVER:
- edits a `SKILL.md` (that's `/promote-skill-proposal`).
...
- `promote <project> <skill>` → write `~/.claude/state/manager-bot-queue/promote-task.txt` for the next user-driven `/promote-skill-proposal` invocation (this skill does NOT execute Claude itself; it just queues). Reply `queued`.
...
- A `sanitize-transferable` rejection happened (visible in `supervisor_verdict.md`'s "Notes for the manager").
...
- Prepend (NOT append) a new dated entry IF the manager noticed a pattern worth shaping the supervisor's behavior:
  ...
  <2-4 sentences. Examples:
    "The last 3 cycles each spent >100k tokens on the deploy step.
     If you see another such run, file a should-fix on dev-cycle's
