...
description: |
 ...
 via skill-chain `--playwright-headless`), writes a structured
...
- **Leave no trace.** Zero files under any repo working tree. Binary artifacts (screenshots, traces, video) go to an out-of-tree state dir; the reviewer-facing findings doc goes to the chain run-log dir (already gitignored). Not repo-scoped: host state you change to make a surface exercisable -- an installed browser or toolchain binary, a shared cache, a global config -- is permitted only when reversible and needed by a check you actually run, and must be named in the findings artifact, never only in your closing message. The servers are always torn down even on exception or timeout. The browser is closed too. A wrapper may reuse one long-lived browser *within a single MCP session* (probe first; do not launch a second). That opt-in is VOID when the harness cannot reattach — Cursor (`cursor-agent -p`) starts a fresh Playwright MCP each invocation, so keep-open reuse leaks a new window per iter. Close on teardown in that case.
...
6a. **Broaden beyond the dev's named guidance: blast-radius / adjacent-surface probing.** After running the dev's named surfaces, treat `tester-guidance.md` as a **FLOOR, not a ceiling**. Derive and exercise your OWN additional test cases from the actual change before collecting findings:
 ...
 - **Read the filing itself before reporting that it omits something.** Broadening past the guidance routinely reproduces an item that is still OPEN in the handoff docs rather than one this cycle closed, and step 1's cross-check reads only the items the cycle flipped to `[x]`, so an open item you stumble onto arrives with no authoritative text attached. Before recording that such an item's severity should change, or that "the filing does not record" a behavior you just observed, READ THAT ITEM'S OWN TEXT in `docs/SPEC.md` / `docs/TODO.md`. Never grade it against the guidance's or the summary's paraphrase: a handoff summarizes an item to orient you and is not a faithful copy, so a novelty claim measured against the summary reports as new what the filing already records. The cost is not just a wasted line; it lands in the findings as a recommendation to downgrade a real item, and the only thing between that and a wrongly-lowered severity is a reviewer who happens to re-read the filing.
...
8. **Tear down.** See **Teardown** — stop both servers and close the browser; assert the documented ports are free and no orphan server processes remain.
...
**Flush a partial record as you go; the terminal write must not be the FIRST write.** Steps 7-9 are the only place the in-chain path touches disk, so until they run there is nothing to find: an abrupt death before them (a harness-level tool-call / serialization error, an agent crash, a context blowout, a hard turn chop) erases the entire stage, including checks that were already established with evidence in hand. The wind-down rule in **Operating principles** does not cover this; that one fires on an approach you can see coming, and an abrupt death has no approach. Nor is the loss confined to this stage: a chain runner that flags a non-zero skill exit skips the REMAINING stages too, so one empty artifact can also cost the iteration its review pass, on a commit that already shipped and is now covered by nothing. So write `tester-findings.{md,json}` to the iteration's log dir as soon as the FIRST check is verdicted, and rewrite both files (same paths, whole-file overwrite) after each subsequent check, with the summary naming the sweep as still in progress. **Every provisional flush records the top-level `verdict` as `degraded`, never `green`.** The per-check `status` values carry the real readings, so nothing is lost by this, and the terminal write in step 9 is the only write permitted to record `green`. Do NOT recompute the top-level verdict over the records written so far: that field is not a summary of what you have checked, it is what downstream reads as the sweep's conclusion over the whole surface, and a `green` derived from the first check of a multi-check sweep is a claim about every surface you have not touched yet. Whatever consumes this file branches on the enum and not on your prose, so a summary saying the sweep is still running does not protect it: a review stage maps `verdict: green` to a non-finding, and an oversight stage running where no review stage did treats it as nothing-to-file and as grounds to skip its own deep pass. `degraded` is the right value on its own terms too, since it already means the surface was attempted but not fully exercised, which is true by construction of every flush before the last. Step 9 then lands the final state over a file that already exists, and its behavior on a run that completes normally is unchanged. One small write per check buys the guarantee that whatever the run established survives the run; this is the in-chain analogue of the per-target flush the looped-standalone drain already mandates.

### Headed vs headless (D2)

Run **headed** when a display is available (e.g. a live `DISPLAY` session), so a human watching the run sees the real interaction; fall back to **headless** when no display exists (CI, a detached cron run, an overnight drain). **Override:** when the chain runner was started with `--playwright-headless` (or the chain YAML sets `playwright-headless: true`; `--tester-headless` / `tester-headless` remain aliases), the prompt includes a force-headless directive — honor it and stay headless even if `DISPLAY` is set (WSLg, local X11, etc.). That override applies to every browser session the chain opens, not only this tester stage. The headed/headless choice never changes which surfaces are exercised — only whether a window is shown. Headless is the safe default when detection is ambiguous.
...
- **Never `await` a permission-gated Web API inside an evaluate; it is the one hang the bullet above cannot bound.** `navigator.clipboard.readText()` / `writeText()`, `geolocation.getCurrentPosition()`, `Notification.requestPermission()` and their kin return a promise that simply never settles when the driving context holds no granted permission and no real user gesture: no rejection, no error, no console output. And unlike a click or a navigate, evaluate-style tools typically expose NO per-call timeout parameter, so "bound every browser interaction" has no knob to turn here; the call hangs until the driver's own idle timeout fires many minutes later, and the run pays that entire timeout in wall clock. Assert on the state the APP writes instead (the field the feature also populated, the toast it raised, the DOM it mutated): a copy-to-clipboard affordance is verified by the text it appends to the visible draft, not by reading the system clipboard back. Grant the permission up front only if the driver supports it; when neither path is available, record the affordance as unverified rather than probing it.
...
- confirms the documented ports have no remaining listener and no orphan server or Playwright-managed browser processes survive (a hung browser that was force-killed for recovery is expected, not a teardown failure).
 ...
 **FILING FLOOR: a `needs-change` is a claim that WORK IS OWED, not a note that something is imperfect.** This stage sits inside a loop whose other end closes items, so its finding rate is an inflow against that outflow, and the loop only converges while inflow stays below it. A real multi-week run measured the divergence: **3.3** non-`pass` checks per iteration against **1.9** items closed per iteration, so the backlog grew **+1.4 items every iteration** and two thirds of the phase's entire item count traced to this stage. The checks were not wrong; they were all filed. Consolidating the backlog afterwards does not help, because that treats the stock while the flow keeps running. So, before recording a `needs-change`:

 - **Name the user-visible consequence in `evidence`**: something a user, client, or operator would notice, or a correctness / data-integrity / financial risk. If you cannot name one, this is not a `needs-change`.
 - **Polish with no such consequence rides on a `pass` check** as an explicit `OBSERVATION (below filing bar):` line in `recommendation`. You are not withholding it: the measurement still reaches the reviewer in the same artifact, it just does not become a backlog item. Style residue, a label that wraps mid-word, a sub-unit rounding no reader consumes, a cosmetic default; these are the intended targets.
 - **Budget: at most 3 `needs-change` per run.** Exceed it only when every one clears the consequence test, and say so in `summary` (`filing budget exceeded deliberately: N findings, each with a named consequence`). A run recording seven has usually stopped distinguishing defects from imperfections. `fail` is never budgeted; a fail is a fail.
 - **Check the open backlog for the same ROOT CAUSE first, not the same wording.** Two findings that differ only in which call site, renderer, or surface exhibits them are one item; recording the second as its own check invites a second id for one defect. The already-filed case is a `pass` citing that item (see the mapping rule below). The costliest pattern the measurement above exposed was one root cause reaching the backlog as eight separate items, each closed separately, each fixing one call site.

 The enum above is CLOSED, and the array key is `checks`, never `findings`. Do not invent per-check statuses: `skipped`, `partial`, `info`, `not-exercisable`, and `known-preexisting` have all been observed in real runs, and every off-enum status (or a renamed array key) silently drops its record from the reviewer's escalation path, because the reviewer machine-parses `checks[]` and acts only on `fail` / `needs-change`. Map the tempting cases back into the enum: a mapped spec you could NOT run this session (an environment or data restriction, a missing fixture, stale auth) is `needs-change` with the can't-run reason and the unlock named in `recommendation` (step 6a's uncovered-gaps rule: the reviewer needs visibility precisely because you could not exercise it); a known, already-filed issue reproduced in passing is a `pass` whose `recommendation` cites the existing backlog item ("already filed as <id>; do not re-file"); and anything else worth carrying forward, including an observation about an INSTRUMENT rather than about the product, rides on the `recommendation` of the `pass` check whose method it concerns, or in `summary` when it fits one line. Those fields are the whole channel: `tester-findings.md` RENDERS this JSON rather than adding to it, so a free-form section written only there reaches no consumer however well measured, and the reviewer's disposal gate cannot dispose what it never parses. Only the top-level `verdict` may be `skipped`; a per-check `skipped` is not a legal value.
