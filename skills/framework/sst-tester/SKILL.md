...
- **Distinguish a true freeze from a merely BUSY page before you name it; do not over-claim.** A click/action that times out is NOT automatically an app freeze or a crash. The most common cause is ACTIONABILITY: the automation waited for the element to become stable/clickable and it never did because the page was continuously re-rendering (e.g. a large data set streaming in, a virtualized list settling), not because the main thread was blocked. Before recording "the browser froze/crashed", CONFIRM with a liveness probe that is immune to actionability: a programmatic click plus a continuous frame-gap / longtask measurement over a few seconds (a real freeze shows a multi-second frame gap or longtask; a busy page stays ~30-120ms), or a trivial `evaluate` that still returns. Report what you actually measured: a true unresponsive main thread → `fail` "<surface> freezes the browser on <action>"; a page that never stabilizes within the budget but stays responsive → `needs-change` "<surface> not interactable within Ns (continuous re-render / slow load)". Never assert "renderer crashed / OOM" when the renderer is alive but busy; that misdiagnosis (a busy page reported as a crash) sends the next cycle chasing a phantom. Include the minimal repro (the exact interaction + data scale) and, when captured, the last console output and whether a framework render-loop warning was logged.
