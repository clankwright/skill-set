...
**Route-2 fallback for a durable executor-queue file — require `Verify:`.** When the spawn-fail fallback (or any Route-2 entry) asks a human to `/exec` / `--process-supervisor-request` a still-on-disk queue file under `~/.claude/state/executor-queue/`, the HUMAN.md entry MUST include a `Verify:` shell check that passes once that basename appears under `~/.claude/state/executor-queue/processed/` (example: `test -f ~/.claude/state/executor-queue/processed/<basename>`). A late or parallel spawn can complete the queue after the fallback is filed; without `Verify:`, the Ask/Reply stays open and misleads ops into re-running an already-processed file. Prefer `## High` (not `## Blocking`) for this class — the cycle is not stopped, only the sync is deferred.
