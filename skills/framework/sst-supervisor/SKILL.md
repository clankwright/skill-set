...
- **Consumption: a prior firing's findings do not re-fire the total trigger.** When a trailing iter's verdict records a fired trigger in its `## Batch-window refinement` block (outcome `edit applied` or `deferred: shape-mismatch`), every `[batch-sizing]` finding from iters at or before that firing iter is CONSUMED by that firing: exclude consumed findings from the total-trigger count (trigger 2), so only findings from iters AFTER the most recent firing count toward M. The same-direction streak (trigger 1) is exempt from consumption -- a streak that persists across a firing is fresh evidence the refinement did not take. Without this rule the total trigger re-fires every subsequent iter on the exact evidence a refinement already answered, because the 20-iter window retains the consumed findings for many iters (observed: a window fired M=5 and applied a chunk-shape refinement, then re-totaled 6 one iter later with 5 of 6 findings identical -- the single fresh finding was a retry-sum artifact whose single-attempt cost was in-band -- leaving the session a choice between a spurious second refinement and a `deferred: shape-mismatch` label that would repeat every iter until the consumed findings aged out).
