...
3. **Transcript keyword scan returns clean.** Search every `<i>_<skill>.txt` **case-sensitively** for any of the following — using word-boundary anchoring (`\b`) on the generic terms to prevent false positives on compound identifiers and prose substrings. Case-sensitivity is load-bearing, not incidental: the generic terms below are matched as UPPERCASE tokens because real Python/pytest failure output emits `ERROR`, `FAILED`, `Traceback`, `Exception` in those exact casings, while benign lowercase prose ("error out", "tests fail as expected", "no regressions") is narration the dev/review skills routinely write on clean runs. A case-insensitive scan trips the gate on that narration and forces a deep-walk-on-clean every time — the documented ~30-50%-extra-turns waste this condition exists to avoid. Do NOT reintroduce a case-insensitive flag here. (The keyword scan runs only after condition #2 has confirmed all exit codes are 0, so a real unresolved test failure is already caught upstream; this scan is the secondary net for the narrow case of a skill that exited 0 yet left an uppercase failure token in its output.)
   ...
   - `Found [1-9]\d* items:` — matches `sst-dev-review`'s §6 "With findings" report line (`Found <N> items: <B> blocker, <S> should-fix`); a findings-filing review always reports N≥1, so any match means findings were filed and a `clean (fast-path)` verdict would be false. The count anchor is deliberately `[1-9]\d*`, not `\d+`: an off-template clean report has been observed opening with `Found 0 items: 0 blocker, 0 should-fix`, and a zero-count match must not abort the fast-path (it would force a deep walk on precisely the clean iterations the fast-path exists to skip, while a genuine findings-filing body is still independently caught by `Review follow-ups` below). Case-sensitive; no word-boundary anchoring needed — the sequence is specific enough to the review §6 report template that it will not appear in non-review output on a clean run.
...
**Runner-recorded contract flags.** The iter MANIFEST may also carry runner-set non-fatal contract-violation flags at the top level (e.g. `batch_pick_missing`, set when the dev shipped a commit without its mandatory `[batch-pick]` block). Read them from the MANIFEST, not by transcript grep, but they are only as complete as the attempt the runner's sentinel capture scanned: the FINAL one. So when the flagged skill's own record carries `retry_count > 0`, confirm it against that skill's own `.retry-<n>.txt` siblings first; a sentinel emitted in a discarded attempt DISCHARGES the flag as a capture artifact, never as the skill's compliance gap. Dispose of a confirmed flag per §7's non-fatal-flag carve-out: a tracked note in `## Notes for the manager`, not an escalation.
...
2. **Re-home the offending item so the next iteration does not re-fail on the identical pick.** Edit `docs/TODO.md` / `docs/SPEC.md`:
   ...
   - **If the failed skill is NOT the dev** (a tester, a review, any stage that runs AFTER the commit) and the dev already exited 0 with its commit pushed, there is no offending pick to re-home: the tree is clean and the item is already closed, so the two bullets above have nothing to attach to, and re-homing a finished item would be wrong. What the failure actually cost is the COVERAGE of the stages the runner skipped. The runner drops every remaining intermediate skill on the reasoning that they "would run against the failed skill's half-done state", which is true of a failed dev and false of a failed verifier, whose downstream stages would have run fine against a complete commit. So an already-pushed commit ships without whichever verification stages the runner skipped, and a literal reading of this step produces zero edits and zero record of that. Re-home the COVERAGE instead: append ONE bounded `## Next up` item naming the un-covered commit SHA and which stages never ran, so a later cycle re-covers it. Dedup first; when an open spec item already carries the same acceptance the skipped stage would have supplied, cite that ID rather than minting a duplicate, and scope the new item to the residual only. Name the skipped stages in the verdict's `## Skill failure` block either way.
...
2. **Other wrappers of the same base.** Run the checker BY ABSOLUTE PATH FROM THE WATCHED PROJECT'S CWD (`python3 ~/Dev/skill-set/bin/check-ssp-sync.py`), never as the bare relative `bin/check-ssp-sync.py`, which only resolves once you have `cd`'d into the base repo. Its two scan roots are `~/.claude/skills` and `./.claude/skills` relative to the CURRENT directory, so running it from inside the base repo points the second root at the base repo instead of the watched project: it then sees only the globally-installed wrappers, reports them all in sync, and says nothing about the project-local wrapper you are in the middle of reconciling. (Observed: a session that `cd`'d into the base repo to make the relative path resolve got `OK: all wrappers reconciled` over three globally-installed wrappers; the identical script re-run from the project cwd reported `DRIFT: 1 wrapper(s)` naming that project's own supervisor wrapper, pinned one base version behind.) It lists every wrapper under those roots whose `base-version:` trails the current base `sst-X`, is unpinned, or points at an unknown base. Wrappers living in OTHER projects are under no scan root and will NEVER appear in its output, so a clean report is not evidence they are current: enumerate them yourself (`ls <projects-root>/*/.claude/skills/*<skill>*/SKILL.md`, or re-run the checker once per project root with `--skills-dir`) before concluding the sync is done. For each stale wrapper:
   ...
   - **In another project you cannot reach from this run:** do not guess. "Mechanical" (a pure `base-version:` bump) is a claim about that wrapper's bytes, which you can still READ: RUN the overridden-surface grep this session, even when an inherited un-dispatched batch asserts the answer. A grep you did not run makes it a judgment reconcile (§5c or a HUMAN.md decision-request), never pin-only - stale stays visible to the checker, falsely-pinned reports reconciled forever.
...
- **Trailing iter set.** The window spans RUNS: it is the 20 most recent iters across the host project's `.skill-runs/` history (the `M=5 in trailing 20` window's outer bound), not just the current run. For multi-iter runs, walk backward from this iter through `<base>/iter_<NN-K>/` directories (K = 1, 2, ...); when the current run's earlier iters are exhausted before 20, CONTINUE into prior `<cwd>/.skill-runs/*/` run directories sorted by name (timestamp-prefixed), newest first, each contributing its iters in reverse order (a single-iter run contributes one); a prior run's HIGHEST-numbered iters enter the window first (a finished 20-iter prior run contributes `iter_20, iter_19, ...`; its earliest iters typically never enter the window at all). Recompose the window from the on-disk `iter_*` directory listing every session; NEVER inherit the previous verdict's `Trailing window scanned` enumeration, which is §3.5.4 outcome bookkeeping, correct only at its own timestamp (observed failure: a session walked a finished 20-iter prior run oldest-first, `iter_07..01`, letting the walk reach a still-older run's aged-out `oversized` lines and re-fire the M-total trigger on phantom counts of 6 and then 5 against a true 2; the next session copied the composition forward shifted by one iter). Stop at 20 or when the host has no older iters; partial windows are fine (the thresholds are minimums, not minimums-of-a-fixed-window). Do NOT stop the walk at the current run's boundary: a run-scoped window resets the longitudinal signal on every new run and has been observed to record contradictory `total in trailing 20` counts (0, 1, and 10) in adjacent verdicts reading overlapping history, flipping the total-trigger evaluation between them.
- **`[batch-sizing]` finding extraction.** For each iter in the trailing set, locate the dev-review transcript (the file whose name matches the chain's review skill — e.g. `01_sst-dev-review.txt` or `01_<proprietary-review>.txt` per the chain definition). **When that iter also carries `<i>_<review>.retry-<n>.txt` siblings (the chain runner writes one per re-attempt after a rate-limit pause or a crash), the un-suffixed `<i>_<review>.txt` is the ONLY authoritative transcript** — it is the attempt that actually completed and produced the iter's review output. Select it by exact name, never by a glob's first hit: `.retry-0.txt` sorts BEFORE `.txt` (`.r` < `.t`), so a `ls <iter>/*<review>*.txt | head -1` silently reads the abandoned attempt, and a fired machine line present only in the completed attempt is then dropped from the window as a phantom no-line iter — the same false-negative class the current-iter receipt audit below exists to catch, arriving through file selection rather than through the emitter. Conversely, do NOT count a line that appears ONLY in a `.retry-<n>.txt`: that attempt did not produce the iter's review. (Observed: an iter whose review was re-attempted after a rate-limit pause left both files on disk, and a first-hit glob over them reported zero `[batch-sizing]` hits for an iter whose completed transcript did carry a tagged line — the pause-and-resume policy makes such sibling pairs routine on long looped runs, so this is not a rare shape.) Grep for lines matching `\[batch-sizing\]` (any position on the line, case-sensitive — the tag is framework-canonical). The primary extraction target is the machine-parseable summary line emitted by `sst-dev-review §2.10` in the format `[batch-sizing] direction=<undersized|oversized> difficulty=<tier> actual=<n>k band=<lo>-<hi>k`; the any-position match also catches legacy prose-embedded mentions. Each match is one CANDIDATE finding, not yet a counted one: **a tagged line carrying neither a `direction=<value>` key nor a standalone `undersized` / `oversized` fire token is a no-fire RECEIPT the review tagged by mistake** (`sst-dev-review` §2.10 forbids the tag on a no-fire iter precisely because this grep is its consumer), so DROP it from the count, record the drop in the §3.5.4 bookkeeping block, and treat that iter as a no-line iter so the current-iter receipt audit below still applies to it. Never recover a direction from receipt prose: the phrases `undersize threshold`, `below band`, and `not oversized` all appear in legal no-fire receipts, so a bare-substring fallback manufactures a direction the review never fired and feeds the same window pollution the discount rule below exists to stop (observed: a no-fire iter emitted `[batch-sizing] no line: actual=177k vs medium band 200-300k, above the 100k undersize threshold`, which an any-position grep counts as a fourth finding in a window where three fired). **A tagged line that QUOTES a machine line the review DECLINED to emit is a DECLINE NARRATION, not a finding.** The drop rule above catches a receipt that OMITS the `direction=` key; this is the mirror case, where an audit bullet reproduces a full `direction=<value> ... band=<lo>-<hi>k` line VERBATIM in order to show what the superseded rule would have fired before the review declined it, so the key IS present and the drop rule never engages. Recognize it structurally, from two conditions that must BOTH hold: the `[batch-sizing]` tag does not LEAD the line's content (narrative prose precedes it, typically with the quoted line inside a code span), AND the same line carries an explicit not-emitted clause (`no line emitted`, `Declined`, `would have produced`, or an equivalent counterfactual). Drop the candidate, record the drop in the §3.5.4 bookkeeping block, and treat that iter as a no-line iter so the current-iter receipt audit below still applies to it. Requiring BOTH conditions is what keeps the any-position net intact for a legacy prose-embedded mention of a REAL finding, which carries no such clause; do not relax to either condition alone, because a genuinely emitted machine line may be preceded by nothing on its line, and the word "declined" appears in legal receipts about a declined BAND SUBSTITUTION rather than about the line itself. (Observed: a review that correctly declined a substitution wrote "Under the pre-<version> rule that substitution would have produced `[batch-sizing] direction=oversized actual=243k band=220-250k` against a pick sitting comfortably inside 200-300k. Declined correctly; no line emitted", and an any-position grep counted it as a fired `oversized` finding for an iter whose authoritative summary line read "in-band, no line". The emitter is behaving correctly there and the narration is useful audit evidence, so the fix belongs on the consuming side rather than in a rule forbidding the review to show its work.) For a genuine machine line, capture the `direction` token from the `direction=<value>` key, or from a standalone `undersized` | `oversized` token on the line when no key is present. Capture the iter's primary difficulty from the `difficulty=<tier>` key when present, else from `iter_manifest.difficulty`, else from the `[picked-difficulty: <tier>]` sentinel in the iter's dev transcript.
- **Reconcile every `oversized` line against the iter MANIFEST before counting it.** The window tunes how much work the dev PACKS, so a line counts only when its `actual=` measures the pick's own cost, and two artifacts routinely inflate it past that: a `cacheReadInputTokens` sum (the ~40x emitter error `sst-dev-review` §2.10 names and warns "pollutes the supervisor's §3.5 trailing-window aggregation with a false `oversized` line"), and a MULTI-ATTEMPT sum, which §2.10 requires the review to add back whenever the dev's `retry_count > 0`, charging the same work to the window twice after a rate-limit resume, overload retry, or turn-ceiling chop. So never take `direction=oversized` at face value. Recompute each one from that iter's MANIFEST as the dev skill's `inputTokens + cacheCreationInputTokens` (the `model_usage` of `skills[0]` only, summed across model entries, `cacheReadInputTokens` EXCLUDED), taking the LARGEST single attempt across that record and its `.retry-<n>.txt` siblings rather than their sum when `retry_count > 0`; drop the line when a residual overshoot still exceeds ~3x the upper edge (measurement error, not a real oversize, which tops out at ~2-3x) or when the MANIFEST is unavailable. Re-admit under the corrected direction AND magnitude: it may become `undersized`, in-band and dropped, or a genuine oversize far smaller than the emitted figure. Record each recomputed line in the §3.5.4 bookkeeping block. The supervisor must never fire a window-refinement trigger, or size a band-edge move, on cost the pick did not cause.
- **Audit the CURRENT iter's no-line receipt before counting it as zero.** The extraction above counts emitted machine lines, so a review that wrongly declares an out-of-band `actual` "in-band" (or writes a receipt with no band edges at all) starves the window with a silent false-negative -- the mirror image of the false-`oversized` pollution the discount rule above guards against. For the iter under review only (each iter is audited exactly once, by its own supervisor pass): when its review transcript contains NO `[batch-sizing]` machine line, recompute `actual` from the iter MANIFEST exactly as the discount rule specifies (the dev skill's `inputTokens + cacheCreationInputTokens`, `cacheReadInputTokens` EXCLUDED, plus earlier-attempt totals from `.retry-<n>.txt` transcripts when present) and compare it against the band for the effective difficulty (re-banded on the commit's primary bracket when the MANIFEST `difficulty` is a queue-top pre-parse, per `sst-dev-review` §2.10). If the recompute lands outside the band under the review's own fire-rules (oversized: above the upper edge by <=~3x; undersized: below the undersize threshold with §2.10's unbatched-queue condition), ADMIT the corrected line into this window's count as if emitted, record it in the §3.5.4 block as `corrected line: [batch-sizing] direction=<dir> difficulty=<tier> actual=<n>k band=<lo>-<hi>k (receipt-error: review declared no-line/in-band)`, and record the receipt error as a finding against the review skill (observed: a receipt quoting `actual=~282k` on a 100-200k-band iter declared it "within band" with no band edges named; the suppressed `oversized` line left the M-total trigger sitting one short of firing). Later window reads count a windowed iter's corrected line from that iter's verdict `## Batch-window refinement` block -- this is per-iter finding data, distinct from the window-COMPOSITION enumeration the trailing-iter-set rule above forbids inheriting. **Compare at whole-`k` granularity, and treat an exact tie with a band edge as IN-band**, per `sst-dev-review` §2.10's tie rule, which this audit defers to: round the recomputed total to the nearest `k` and admit a corrected line only when the rounded value lands strictly outside the rounded band. A raw total a few hundred tokens past an edge rounds ONTO it, so admitting it would both write a self-contradictory corrected line (an `actual=200k band=100-200k` that names no overshoot) and let a sub-0.1% measurement artifact push the hard-counted M-total across its threshold. That is the same "never fire a window-refinement trigger on a measurement error" perimeter the discount rule above defends, arriving through the audit instead of through the emitter. (Observed: an iter that hit its declared window-target summed to 200,164 against a 200k upper edge while the trailing window stood at exactly M-1; a mechanical "above the upper edge" read of this bullet would have fired a band-edge refinement against the best-sized iter in the window, on 164 tokens.)
- **Consumption: a prior firing's findings do not re-fire the total trigger.** When a trailing iter's verdict records a fired trigger in its `## Batch-window refinement` block AND a disposal of it -- an edit, a stated deferral, or a reasoned decision that the right response is to change nothing, which §3.7.3 authorizes in as many words -- every `[batch-sizing]` finding from iters at or before that firing iter is CONSUMED by that firing: exclude consumed findings from the total-trigger count (trigger 2), so only findings from iters AFTER the most recent firing count toward M. Only a firing whose verdict records no disposal at all consumes nothing; the disposal's SHAPE never decides this, because a firing answered with "the mechanism is working and the phase is simply large" has spent its evidence exactly as fully as one answered with an edit. The same-direction streak (trigger 1) is exempt from consumption -- a streak that persists across a firing is fresh evidence the refinement did not take. Without this rule the total trigger re-fires every subsequent iter on the exact evidence a firing already answered, because the 20-iter window retains the consumed findings for many iters, leaving the session a choice between a spurious second refinement and a deferral label that repeats every iter until they age out.
...
3. **Stable-termination override (default K=10):** if the most recent K iters all carry zero `[batch-sizing]` findings, the loop is in stable-termination mode. Suppress trigger evaluation entirely; return `no refinement needed (monitoring (K=<n>))`, where `<n>` is the current clean-streak length: read the most recent trailing verdict's `## Batch-window refinement` block and increment the `monitoring (K=<n>)` value it carries, or start at the streak length that just satisfied K when no prior block carries one. **The clean K-streak ALONE engages the mode; do NOT additionally require that a prior verdict already recorded a monitoring block.** That second conjunct is a bootstrap deadlock: the only writer of a `monitoring (K=<n>)` block is §3.5.4 recording the outcome THIS step returns, and this step returns `monitoring` only when the mode is already engaged, so a mode gated on a pre-existing block can never be entered from a cold start. That in turn makes §3.5.4's "a clean K-streak produces a single `monitoring (K=<n>)` block per iter (incrementing)" unreachable prose, and permanently forfeits the scan cost the override exists to save. (Observed: a 20-iter run whose only two `[batch-sizing]` findings both landed in its first half reached an exact 10-iter clean streak at its final iter, with all twenty verdicts recording `below threshold` and not one recording a monitoring block; a mid-run verdict had already named the missing prior block, rather than the streak length, as what was holding the override off.) Re-engage the moment a `[batch-sizing]` finding lands again (the K-streak resets to zero on the next finding).
...
1. **Adjust a band edge** - when a streak trigger fires in a single difficulty band with a single direction. Move the edge by a single increment (~10-20% of the current edge value), in the direction the PICK's cardinality implies. For BATCH picks (>1 item) the packing presumption holds: undersized raises the tier's lower edge (the floor was too low to motivate batching), oversized lowers its upper edge (the ceiling permitted over-packing). For SOLO picks no smaller batch existed, so oversized is not a packing decision and lowering the ceiling prescribes an impossibility: RAISE the upper edge, or prefer kind 2 when what overshot was the dev's own declared estimate for that work shape. Cardinality is a read off the `[batch-pick] <N> items @ <tier>` block. Do NOT cross-tier (an `[easy]`-band edge change must stay inside `[easy]`'s neighbors; never let `[easy]`-upper > `[medium]`-lower).
...
2. **Flat-backlog window (default M=8):** across the `M` most recent samples **that carry the newest sample's `phase=`**, `phase_open` at the newest is greater than or equal to `phase_open` at the oldest, AND at least `M` such iterations shipped commits in that span; fewer than `M` same-phase samples cannot fire it, because `phase_open` counts one phase's section and a window spanning a phase change compares unrelated backlogs. This trigger reads `phase_open` only, which the emitter is required to RE-COUNT from the spec each time rather than carry forward, so treat two identical consecutive readings as data and not as a copy: if a run of identical values coincides with iterations that closed items, the suspicion belongs on the emitter, and the check is one `grep -c` against that phase's section, which you should run for the current iter before counting its sample (a stale `phase_open` is a receipt error of the same class as the `closed` mismatch above, and gets the same `should-fix` against the review skill). It catches the regime the streak trigger misses, where `filed` and `closed` alternate around parity: a phase absorbing `M` productive iterations without its open count falling is not draining, whatever the per-iter arithmetic says.
...
When an action the cycle cannot perform autonomously -- writing a secret outside the repo, granting access in a third-party UI (e.g. GitHub Actions secrets, cloud IAM), signing a legal agreement, deciding whether a re-scoped feature is still wanted, or anything else needing a human's credentials or authority -- resolves a finding OR is named by a chain skill in its own transcript (since Phase 54 no chain skill may write `docs/HUMAN.md`, so this layer is its only carrier), append to `docs/HUMAN.md` instead of `docs/TODO.md` or `docs/FUTURE-WORK.md`.
...
**Route 1 - autonomous (dispatch to `sst-executor`).** Use when the follow-up is reversible/local framework maintenance with an unambiguous done-state: refresh the runtime skill copies (`bin/install-skills.sh --yes`), a mechanical `base-version:` bump on a wrapper whose base change touches nothing it overrides, run a diagnostic. Specify each action NON-INTERACTIVELY, naming the confirm-bypass flag: the executor runs detached with no stdin, so a prompting command reads EOF and does nothing, printing completed-run-shaped output. Collect ALL such follow-ups from this session into ONE request file and spawn the executor ONCE:
...
**Route-2 fallback for a durable executor-queue file — require `Verify:`.** When the spawn-fail fallback (or any Route-2 entry) asks a human to `/exec` / `--process-supervisor-request` a still-on-disk queue file under `~/.claude/state/executor-queue/`, the HUMAN.md entry MUST include a `Verify:` shell check. A late or parallel spawn can complete the queue after the fallback is filed; without `Verify:`, the Ask/Reply stays open and misleads ops into re-running an already-processed file. **Key the check on the requests' actual end state, NEVER on the `processed/` marker.** Write a conjunction of the concrete post-conditions the batch was supposed to produce — the version string each synced file should now carry, the pin each reconciled wrapper should now hold — so the check can only pass if the work really landed. A `test -f ~/.claude/state/executor-queue/processed/<basename>` check is NOT acceptable: archival is bookkeeping, not evidence of execution. (Observed: a supervisor's `claude --print` executor spawn died immediately on "You've hit your session limit", and the queue file was moved into `processed/` anyway with not one of its three requests run — the runtime skill copies and both wrapper pins were still at their pre-bump values afterwards. A `processed/`-keyed `Verify:` would have passed on the spot and closed the entry with the whole batch outstanding.) For the same reason, **never treat a spawn's exit code or the appearance of the `processed/` marker as confirmation** that a dispatched batch ran: after the single §5c spawn returns, re-read the files the requests were supposed to change, and fall through to this Route-2 fallback when they are unchanged. Re-file the batch as a FRESH queue file (a new `<utc>_supervisor-request.json` under the write-path in §Output rules) rather than reaching into `processed/` to move the old one back; that directory is the executor's state, not the supervisor's. Prefer `## High` (not `## Blocking`) for this class — the cycle is not stopped, only the sync is deferred.
