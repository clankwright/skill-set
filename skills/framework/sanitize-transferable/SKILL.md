...
This skill **never silently edits** the target. It reports findings and (optionally) writes a sibling `.sanitized.md` file the user reviews and promotes via `/promote-skill-proposal`.

## When to invoke

- Before the supervisor writes a transferable proposal: `/sanitize-transferable <draft.md>`.
- Before `/promote-skill-proposal` applies a transferable proposal: it calls this skill internally on the proposed body.
...
Promote with: /promote-skill-proposal <target>.sanitized.md
```
...
Scanned at: <utc-iso>  ·  By: sanitize-transferable v<version>
```
...
- **Never edit the target file.** Only the user, via `/promote-skill-proposal`, applies sanitized rewrites.
...
There is no `--force` flag. If you want to ship something without scanning, don't call this skill — but then `/promote-skill-proposal` will refuse the transferable promotion (it requires this skill's report sibling-file to exist with timestamp newer than the target).
