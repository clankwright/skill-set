...
description: Single-session driver for a multi-iteration skill-chain run. Spawns `bin/skill-chain.py --chain <name> --loop N` as a subprocess via `bin/orchestrate-chain.py`, watches the live event stream, posts Telegram updates at session start, each iteration boundary (commit + per-iter spend + cumulative), every rate-limit pause/resume, supervisor escalation, and session end. Honors a `--max-budget-usd` halt and a `--max-cycles` halt independently of the chain's own loop count. Distinct from sst-manager (cron-based, multi-project, periodic) and from sst-agent-orchestrator (in-process planner inside one user request). The proprietary counterpart supplies the watched-chain name, Telegram chat ID, and budget defaults.
user-invocable: true
version: 1.0.0
...
- **One subprocess, one session.** The orchestrator wraps exactly one `bin/skill-chain.py` invocation. To watch a different chain, spawn a second orchestrator. Don't multiplex multiple chains inside one session.
...
- **Telegram is best-effort too.** A failed Telegram send (network, bad token, missing chat ID) prints to stderr but does not abort the orchestrated run. The chain itself is the source of truth; Telegram is a courtesy stream.
- **No retries on the chain runner.** Rate-limit pause-and-resume happens INSIDE `bin/skill-chain.py` (Phase 13). The orchestrator just observes and forwards. If the chain exits non-zero, the orchestrator exits non-zero.
- **Iteration boundaries are the unit of accountability.** Per-iteration `MANIFEST.json` files are the canonical record of cost, commit SHA, and supervisor verdict. The orchestrator reads those — it does not estimate costs from streamed events, and it does not parse skill output for project facts.

## Inputs

The orchestrator accepts the user's intent in either form:

- **Direct**: the user invokes `/sst-orchestrator` with a chain name and caps on the command line. Example: *"run dev-cycle-with-review-looped, halt at $30 or 5 iterations, use the manager telegram env at ~/.config/manager-telegram.env"*.
- **Configured**: the proprietary counterpart (e.g. `<project>-orchestrator`) declares the watched-chain + caps + telegram-env path in its frontmatter or body, so the user only types the skill name.
...
2. Confirm `bin/orchestrate-chain.py` is present at `~/Dev/skill-set/bin/orchestrate-chain.py` (or `<repo-root>/bin/orchestrate-chain.py` when working inside this repo). If not, the framework is not installed cleanly: bail with a clear error pointing the user at `bin/install-skills.sh`.
3. Confirm the chain definition resolves: `bin/skill-chain.py` will look in `<cwd>/.claude/chains/<name>.yaml` first, then `<repo>/chains/<name>.yaml`. If neither exists, fail loud — there is no point spawning an orchestrator over a non-existent chain.
...
3. Defaults: no budget cap, no cycle cap, loop = 1 (degenerate but legal — the orchestrator works fine over a single-iteration chain).

Validate: budget > 0 if set, cycles >= 1 if set, loop >= 0.

### 2. Spawn the orchestrator helper

Build the command and invoke via Bash:

```bash
bin/orchestrate-chain.py \
  --chain <name> \
  --loop <N> \
  --max-budget-usd <X> \
  --max-cycles <N> \
...
| :---  | :---    | :---                |
| session-start | orchestrator startup | chain name, iter count, caps, log dir, started_at |
...
```
sst-orchestrator: chain=<name> iters=<completed>/<requested> cost=$<X.XX> exit=<rc>
  log: <run-dir>
...
- **Never run `bin/skill-chain.py` directly.** Always go through `bin/orchestrate-chain.py`. The whole point is the watcher; bypassing it loses the budget gate and the Telegram stream.
- **Never edit a `SKILL.md` from inside the orchestrator.** That's `sst-supervisor` + `/sst-promote-skill-proposal`. The orchestrator is read-only across `.claude/skills/`.
- **Never spawn a second orchestrator from inside one.** Recursion is meaningless here; one session = one chain. If the user asks for two parallel runs, they invoke the skill twice.
...
- **Never silently drop a Telegram failure.** The helper writes failed sends to stderr with a `[orchestrator]` tag; the skill should leave that visible rather than swallowing it. If the user wants no Telegram at all, they pass `--no-telegram`.
