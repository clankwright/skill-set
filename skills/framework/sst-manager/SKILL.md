...
The manager is the third-and-final loop. It runs in four modes:
...
1. **Mode dispatch.** Parse the input for mode tokens. If `--process-feedback <queue-file>` is present, jump to §On-demand feedback routing and skip §1–§6. Else if `--process-command <queue-file>` is present, the next token is the queue-file path; jump to §On-demand command routing and skip §1–§6. Else if `--plan` is present (with no other mode token), jump to §Planner mode and skip §1–§6. Else continue with periodic mode below; periodic mode's §3 may auto-transition into planner-mode in-process when conditions hold.
...
3.7. **Take a backlog reading for the project's active phase.** Two figures: how many items are open in it now (the `- [ ]` count under that phase's SPEC section, FUTURE-WORK items excluded per step 3), and which way that number has moved. Prefer the newest `supervisor_verdict.md`'s `## Backlog growth` block, which already carries the trend and the per-iteration filed-versus-closed deltas; when no verdict carries one, fall back to comparing today's count against the count recorded in your own previous digest for this project. Note whether the phase's `## Next up` subsection carries a freeze banner. This reading is what makes §4's backlog bullet possible, and it is the one number a commit list cannot show: a tick can ship five commits while the phase it ships into grows, and those two facts look identical in `SHIPPED`.
...
**Blocked-item gating (re-homed from sst-dev-cycle's old `[blocked-on-human]` read in Phase 54):**

The dev cycle no longer reads `docs/HUMAN.md`, so the manager is now responsible for keeping any SPEC ID named in an open HUMAN.md `Blocks:` line off the pickable top of `## Next up`, so the dev cycle never picks a blocked item.

1. From the open `## Blocking` entries collected above, gather every SPEC ID listed on a `Blocks:` line (ignore `Blocks: none`). Call this `blocked_ids`.
2. Scan the project's `docs/TODO.md > ## Next up`. For each queued line whose leading `<phase>.<n>` ID (or whose prose) names an ID in `blocked_ids`:
   - If it is not already tagged, append a ` (blocked: <H-ID>)` tag to the end of the line and relocate the line to the BOTTOM of `## Next up`, below all un-blocked entries. The dev picks the top un-blocked entry, so a blocked item parked at the bottom is never the pick while actionable work remains.
   - If every queued item is blocked, leave them parked and tagged; the digest's NEEDS YOU section surfaces the blockers so the human can unblock them.
3. When auto-verify (above) closes the blocking HUMAN.md entry, also strip the matching ` (blocked: <H-ID>)` tag from any `## Next up` line so the item becomes pickable again on the next cycle.

This is a narrow periodic write to `## Next up` (tag + relocate blocked lines only), distinct from the append-only feedback/planner exceptions; it never deletes a queued item, never renumbers, and never touches `## In flight` or `## Just shipped`.
...
• Backlog: <N> open in <plain name of the current phase> · <draining|flat|growing> (<±k> since <Month D>)<, scope frozen when the banner is set>
...
- **The backlog bullet is mandatory whenever step 3.7 produced a reading**, and it is the only STANDING line that may repeat unchanged tick after tick: a flat backlog IS the news. Say `draining` when the count fell, `growing` when it rose, `flat` when it held, and give the delta with the date it is measured from so the user can see the rate rather than a snapshot. Add `scope frozen` when the phase carries a freeze banner, which reads as reassurance rather than as a warning: it means the phase stopped taking on new non-critical work and can now finish. **When the backlog has grown across two or more consecutive ticks, promote it to a NEEDS YOU line** phrased as the decision the user actually owns ("the current phase has grown for 3 checks: merge it as-is, cut its scope, or accept the growth"), because at that point no autonomous stage can fix it and continuing to report it under STANDING buries the one thing that needs a human. Do not name the internal mechanisms (filing budgets, freeze thresholds, machine lines) in any of this; the user wants the trend and the decision.
...
**(e) HUMAN.md blocker entry -- append to `docs/HUMAN.md`.** **HUMAN.md admission test (hard gate):** route here ONLY when the action is one ONLY THE HUMAN can perform or decide -- an out-of-band credential the framework lacks (a secret, a third-party-UI or cloud-IAM grant, a lapsed production-access login), a legal/contractual signature, OR a product/scope decision that is the human's alone. If any autonomous actor (the project's dev cycle, or `sst-executor` for reversible/local upkeep) could perform or decide it, it is NOT a HUMAN.md item; prefer outcome (a) `## Next up`, (b) a SPEC item, (c) a manager-translated manager-notes entry, or `docs/FUTURE-WORK.md`. **When uncertain whether an action is human-only, it is NOT** -- keep the borderline entry out. Use when the feedback body maps to a human-only action the cycle cannot perform: "I can't provision the required secrets yet", "waiting on legal approval", "need to grant access in the cloud console". Examples: the user is telling the manager they cannot do something a pending SPEC item requires, or they want to record that a specific action is blocked on an external dependency. Append to the chosen project's `docs/HUMAN.md` under `## Blocking` (if the action actively stops a SPEC item) or `## High` (non-blocking prerequisite):
...
## On-demand command routing (`--process-command <queue-file>`)
...
1. Read the queue file (`<queue-file>`). Required JSON fields: `command` (verb string), `args` (list), `from_chat_id`, `received_at`. If the file is malformed, missing, or already in `processed/` (idempotency), reply via Telegram `Already processed (or queue file missing); ignoring.` and exit 0.
2. Read the proprietary counterpart's frontmatter / body for the configuration (watched-projects, objectives-path, telegram-env). Fail fast with a stderr message if `watched-projects` is absent; `telegram-env` is optional (see §0.4 fallback chain).
3. Source the Telegram env file as in §0.4.
4. Resolve the target project: the command's `args[0]` is the project token. Match it against `watched-projects[*].name`. If no match, reply via Telegram `Unknown project token '<token>'. Run /projects to see known tokens.` and exit 0 (not an error — bot routing is best-effort).
5. Move the queue file to `processed/<basename>` after reading so a concurrent periodic-mode drain does not double-process it.

### B. Dispatch to the verb handler

Match `command` against the supported verbs below. For an unrecognized verb, reply via Telegram `Unknown command '<verb>'.` and exit 0. Each handler:
- Executes in the context of the resolved project path (read the relevant project files from `<project-path>/docs/` and `<project-path>/.skill-runs/`).
- Sends its reply via `bash bin/notify-telegram.sh` (with `TELEGRAM_LABEL=<persona>` set, so the label prefix is consistent with periodic-mode outbound).
- Appends a one-line outcome to `~/.claude/state/manager-notes.md` via `echo "<!-- cmd: <verb> @ <utc-iso> → <outcome> -->" | bin/manager-write-state.py --source observation`.

**`status`**: Read the most recent digest file for this persona (`~/.claude/state/manager-digests/<persona>_*.txt`; pick by lexicographic-last filename, which equals chronological-last since the filename carries the UTC ISO timestamp). Send the digest body via `bin/notify-telegram.sh` — long bodies are chunked automatically by the helper. If no digest file exists, reply `No digest yet for <persona>; try running /<persona>-manager to generate one.`
...
**`ping`**: Compute a brief health summary: (a) last digest timestamp (basename of newest `<persona>_*.txt` in `manager-digests/`, or `none`); (b) queue depth (count of `*.json` files in `manager-bot-queue/` not under `processed/`); (c) blocking HUMAN.md entries (count of open `[ ]` items in `<project-path>/docs/HUMAN.md > ## Blocking`, or 0 if the file is absent). Reply: `pong from <persona>: last digest <timestamp>, queue depth <n>, <n> HUMAN.md blocking entries.`

### C. Exit cleanly

Stdout: `command: routed <verb> for <project> (chat <from_chat_id>)`. Exit 0.

If the Telegram send fails, retry up to 3 times with exponential backoff (the helper already does some retry internally). After exhaustion, write a `.error` sibling next to the queue file in `processed/` with the failure reason and exit non-zero — the reply was lost but no state was corrupted.
...
- **Always reply before ending; never defer the ask.** Every `--process-feedback` / `--process-command` run MUST terminate by sending exactly one outbound Telegram message via `notify-telegram`: either (a) the outcome/result, or (b) a DECISION-REQUEST when an outward/irreversible step (git push, deploy, destructive op) needs human approval before finalizing. A decision-request MUST be sent BEFORE the run ends — never deferred to "once you answer." When a decision is pending, the queue file stays in place (unprocessed) so the human's reply can resume it. Ending a run having neither acted nor asked is a contract violation. Mirror the `sst-executor` tier-2 pattern: ask = send the Telegram approval-request now, then exit.

- **No `git commit` / `git push` / SSH / curl-against-prod in watched projects.** The manager never commits, pushes, or talks to live services *in the projects it watches*. Write surface there is bounded to: its own state files (`manager-cursors.json`, `manager-notes.md`, `manager-digests/`), the Telegram outbound, `<objectives-path>` `[ ]` ↔ `[x]` toggles, on-demand mode's `docs/TODO.md > Next up` appends + `docs/SPEC.md` appends in watched projects, AND planner-mode's `docs/TODO.md > Next up` appends of `[unconfirmed:*]` lines (with a `<!-- planner-id: <id> -->` marker), AND periodic §3b's `docs/HUMAN.md` auto-verify rewrite plus its `## Next up` blocked-item tag-and-relocate (Phase 54 gating; blocked lines only). All TODO/SPEC scoped exceptions are APPENDS ONLY (modeled on the existing `objectives.md` `[ ]` ↔ `[x]` exception): never edits an existing item, never deletes, never renumbers, never modifies `## In flight` or `## Just shipped` or any phase heading text, never reorders phases, never commits or stages the change. The one carve-out is §3b's blocked-item gating, which MAY add a ` (blocked: <H-ID>)` tag to, and relocate within `## Next up`, any line whose SPEC ID is named in an open HUMAN.md `Blocks:` entry (never deleting or renumbering it), and strip that tag when the blocker closes. The dev cycle's normal §1 pick is what eventually ships the queued item; the manager just inserts the line. **Planner-mode appends** are further bounded by §Planner mode β re-entry guard (one outstanding `[unconfirmed:*]` batch at a time per project) and by the `[unconfirmed:`-prefix that the user must clear manually before the dev cycle picks the item — the planner cannot silently broaden the agenda.
...
- **On-demand feedback mode is single-feedback-per-invocation.** `--process-feedback` accepts exactly one queue-file path and routes exactly one outcome. Batch routing (multiple queue files in one invocation) is a separate concern that belongs to the periodic-mode drain, not on-demand.
- **On-demand command mode is single-command-per-invocation.** `--process-command` accepts exactly one queue-file path and dispatches exactly one command. Batch command processing belongs to the periodic-mode drain.
- **On-demand modes never compose a full status digest.** Neither `--process-feedback` nor `--process-command` compose a new digest. The `status` verb in command routing reads and resends the *most recent existing* digest file; it does not regenerate one. Digest composition is a periodic-mode concern.
