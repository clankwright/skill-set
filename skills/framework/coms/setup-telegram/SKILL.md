...
# Generated by setup-telegram on <utc-iso>.
...
(
  ...
  curl -sS --max-time 10 \
    ...
    -d "$(python3 -c 'import json,os; print(json.dumps({"chat_id": int(os.environ["TELEGRAM_CHAT_ID"]), "text": "setup-telegram: outbound OK", "disable_web_page_preview": True}))')" \
    "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
  | python3 -c 'import json,sys; d=json.load(sys.stdin); print("sent" if d.get("ok") else ("fail: " + json.dumps(d)))'
)
```

On `sent`: ask the user to confirm they see the "setup-telegram: outbound OK" message in their target chat. On `fail`: the response JSON names the reason (`chat not found` → wrong chat-id; `bot was blocked by the user` → user blocked the bot; `Forbidden: bot is not a member of the group` → re-add bot). Fix and retry. **Never commit the env file to diagnose this.**
...
```bash
echo "setup-telegram: outbound OK (via notify-telegram.sh)" | \
