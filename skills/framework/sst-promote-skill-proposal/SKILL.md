...
/sst-sanitize-transferable /tmp/<skill>.draft.md --project-context <proprietary-supervisor-SKILL.md>
```
...
- **Transferable promotions never bypass `sst-sanitize-transferable`.** No `--force` flag exists. If the proposal lacks a fresh findings file (timestamp newer than the target), this skill refuses — re-run the sanitize skill first.
