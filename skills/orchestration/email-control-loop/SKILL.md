...
- **State dir**: `<project>/data/email-control-loop/`. Per-cycle state files as `<utc>_<cycle-id>.json`. The skill reads the input state file and writes back a possibly-updated state file at the same path (or a new path if the caller passes `--out`).
