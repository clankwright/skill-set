---
name: dev-review
description: Post-cycle second-pass review of the last `/dev-cycle` commit on any project. Reads what shipped (code + tests + spec + TODO + docs), evaluates it against the spec item it closed along several axes (spec parity, correctness, coverage, discoverability, production verification, security, style, performance), and appends concrete follow-up items to the project's spec AND the handoff TODO's "Next up" if critical, blocking, or medium-to-major gaps are found. If nothing substantive turns up, leaves both unchanged and reports "clean." Does NOT fix issues — only names them and schedules them as spec work for the next `/dev-cycle`. Pair with `/dev-cycle` (chained via `bin/skill-chain.py dev-cycle dev-review`).
user-invocable: true
version: 1.0.0
---
...
One invocation = a critical second pass over the last shipped `/dev-cycle` commit. This skill's only output is either (a) a concise `Review follow-ups` block appended to the project's spec as the next cycle's work, or (b) an explicit "clean" report. **Do not fix code here** — except for plainly wrong doc typos that fit in the same commit. Fixes run through `/dev-cycle` on a later invocation.
...
This skill reads `docs/SPEC.md` and `docs/TODO.md` end-to-end on open and may write to both on close (under §4). Severity bar and process are unchanged from the rest of this skill; the only addition is that every blocker/should-fix you file in the spec also gets mirrored as a one-line entry in `TODO.md`'s `## Next up` so the next `/dev-cycle` picks it up without re-scanning the spec. Both files commit together in §5 if anything was added.
...
4. Identify the commit under review:
   ...
   ```
   If that commit is a skill edit, a docs-only commit, a merge, or otherwise **not** a real dev cycle, walk backward with `git log --format='%H %s' -20` until you find the last real `/dev-cycle` commit. Common scope tags the cycle uses: `Auth:`, `UI:`, `Docs:`, `Tests:`, `Deploy:`, `Infra:`, or a feature-area tag. Review **that** commit — do not review skill/docs-only commits.
...
- Locate the project's spec (same file `/dev-cycle` updates — common locations: `docs/SPEC.md`, `docs/ROADMAP.md`, `docs/<project>_SPEC.md`, `TODO.md`, `README.md`). Grep for the section the cycle's commit message references.
...
- **Test count:** the commit message claims `old → new`. Run the project's collect-only equivalent (`pytest --collect-only -q | tail -3`, `jest --listTests`, `go test -list '.*'`) and confirm the current total matches the `new` number. If the absolute is off but the delta is right, it's cosmetic — don't file unless it's misleading.
...
Walk the `/dev-cycle` transcript (or reconstruct from the diff):
...
**Spec.** Open the project's spec file (same one `/dev-cycle` updates). Under the sub-section the cycle touched, append a **Review follow-ups** subsection. Format:

```markdown
**Review follow-ups (open — schedule as the next `/dev-cycle` cycle):**
...
- One checkbox per finding; do not bundle. A later `/dev-cycle` will pick the top unchecked item (or a bundled chunk, if it uses a chunk-sizing rule).
...
Order: blockers first, then should-fix, then any pre-existing entries (push pre-existing entries down — review-spawned items get priority). Don't touch `## In flight` or `## Just shipped`; those belong to `dev-cycle`.
...
**Never deploy.** This skill does not touch production. The follow-ups are spec-only; actual fixes go through `/dev-cycle`.
