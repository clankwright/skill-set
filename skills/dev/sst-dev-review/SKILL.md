...
**When a `[batch-pick]` cites a work-shape carve-out this review finds SATISFIED, band `oversized` against `max(tier ceiling, carve-out ceiling)` and `undersized` against the tier band and its published threshold always.** A carve-out is a licence to spend past the tier, never a cap that tightens it and never an obligation to spend the extra. A cited carve-out whose ceiling sits at or below the tier's is a no-op (decline the substitution; the tier governs both rules). A cited carve-out that is NOT satisfied does not move the band: band on the tier and report the unmet condition separately. Name both the compared band and the tier band in the §6 receipt. The machine line, when one fires, still carries `band=` for whichever band the comparison actually used.
