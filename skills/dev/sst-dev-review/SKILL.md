...
**The rule:** `filed_non_blocker <= max(1, closed)`, and never more than **3** new non-blocker IDs in one review however large `closed` is. `[blocker]` findings are exempt and uncapped: a blocker is by definition what stops the next release, and rationing those is how a queue-hygiene rule turns into a shipping defect.
