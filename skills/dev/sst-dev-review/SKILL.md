...
**The two-tier bar binds every writer of the queue, not only this stage.** A pick queue is a list of what a cycle will do next, so a third tier below `[should-fix]` is a category error there rather than a harmless extra label: the moment the real defects above it are closed, a `[nice-to-have]` reaches the top and is picked, implemented, tested and reviewed at the same cost as a defect, and the review of THAT cycle files its own follow-ups underneath it. Polish is not thereby forbidden, it is parked: route it to `docs/FUTURE-WORK.md` (§4), where a human decides whether it is ever worth a cycle. So do not file a third-tier item; do not accept one from another stage as a queue entry; and when you find existing third-tier entries already in the spec or `## Next up`, move them to `docs/FUTURE-WORK.md` as a §5 edit (no new IDs, no budget charge) and say how many you moved in §6. Upgrading one is legal only on evidence about its IMPACT (a tester measurement, a reachability argument, a newly-discovered consumer), never on the strength of its tag or its position in the list.

## Filing budget: the backlog may not outgrow the closure rate

The severity bar decides whether a finding is REAL. The filing budget decides whether it becomes a QUEUE ITEM this iteration. Those are independent gates, and the second one runs against most reviewers' instincts: a stage that files every real finding it holds makes the backlog track its own observation rate, and since each cycle's commit is a fresh surface for the next review, the queue then has no fixed point. Measured on a real multi-week run: 46 new IDs minted across 21 iterations against ~1.9 items closed per iteration, so the active phase sat at ~60 open items for a week and its definition grew about as fast as the work drained. The phase cannot be finished, merged or shipped in that regime, and not one review in it was wrong.

**Definitions (compute both before §4; both are cheap and the batch-coherence axis has already read the inputs):**

- `closed` = the number of SPEC `- [ ]` to `- [x]` flips in the cycle commit under review (`git show HEAD -- <spec-file> | grep -c '^+.*- \[x\]'`, reconciled against the `[batch-pick]` block).
- `filed` = the number of NEW `<phase>.<n>` IDs you are about to allocate.

**The rule:** `filed_non_blocker + dev_filed <= max(1, closed)`, and never more than **3** new non-blocker IDs in one review however large `closed` is.

**`dev_filed` belongs in that inequality, and leaving it out is the exact failure this clause now exists to prevent.** The queue has TWO writers per iteration: the dev files under its own cap (`sst-dev-cycle` §6 step 4) and then you file under yours. Capping each writer separately bounds neither the queue nor the phase, because the two caps STACK. Observed over 10 iterations of a real run: every iteration was individually compliant on BOTH sides, 12 items were closed, and the phase still grew from 4 open to 6. Nobody broke a rule; the rules simply did not add up to the property they were written for. So read `dev_filed` off the dev's close report FIRST (it commits before you run), subtract it, and file against what is left. When the dev has already spent the allowance your budget for new IDs is zero, and everything you found goes to `docs/FUTURE-WORK.md` or strengthens an open item. A `dev_filed` you cannot resolve is treated as spending the whole allowance, not as zero: an unknown must not buy you headroom. `[blocker]` findings are exempt and uncapped: a blocker is by definition what stops the next release, and rationing those is how a queue-hygiene rule turns into a shipping defect.

The absolute cap of 3 is a default, tunable by a project's proprietary wrapper on the same terms as the freeze threshold below (tighten freely, loosen only with the reason stated in the wrapper's prose). It is derived, not arbitrary: it is roughly the number of items a cycle of this shape closes at its most productive, so a review that fills it is matching the best case rather than the average. A project whose cycles reliably close more per commit can raise it to that number and still satisfy the net-nonincreasing rule; a project running one small item per cycle should lower it to 1 and lean on `docs/FUTURE-WORK.md`.

**Overflow goes to `docs/FUTURE-WORK.md`, not to the floor.** Rank the real non-blocker findings by user-visible impact, file the top `min(max(1, closed), 3)` as queue items, and route the remainder to `docs/FUTURE-WORK.md` under `## Deferred review findings > ### <Phase context>` (create either heading if absent, placing the section above `## Future / human-handled` to match `templates/FUTURE-WORK.md`): one line each, carrying the SAME evidence you would have filed (file:line, mechanism, proposed fix, the commit reviewed), with no `<phase>.<n>` ID and no `## Next up` mirror. They stay recoverable, since a human, the manager, or the planner promotes them when the phase has room. Parking is bookkeeping, not suppression.

**What the budget must never do:**

- Do not raise the severity bar to fit the budget. A finding that cleared the bar yesterday still clears it today; it goes to FUTURE-WORK, not to nothing.
- Do not downgrade a blocker to a should-fix, or relabel a should-fix as polish, to make the arithmetic work. The blocker exemption exists precisely so that pressure has no reason to exist.
- Do not split one root cause across several IDs to look productive, and do not fuse two unrelated causes into one ID to fit the cap. The strengthening rule in the operating principles governs here and is the cheaper move anyway: **strengthening an open item allocates no ID and therefore costs no budget.** A review that strengthens four items and mints none has spent nothing and improved the backlog.
- Do not carry unspent budget forward, and do not treat it as an allowance to fill. It is a ceiling: `closed=3` does not mean file three.

**Deduplicate before you allocate, and receipt what you compared.** The operating principles already make "strengthen an open item" the default over minting an ID; the budget makes that default load-bearing, so the search becomes a required step with a receipt. Before allocating any ID, search the open backlog by MECHANISM (the function, the module, the data path), not by the symptom's wording, and record in §6 what you searched and what came back: `dedup: searched <n> open items on <keys>; strengthened <ids>; <n> minted`, or `dedup: searched <n> open items on <keys>; no mechanism match`. A review with no dedup receipt has skipped the step, and the receipt is what lets a later audit tell a genuinely new mechanism from a fifth symptom of an item already open.

## Phase freeze: a phase's scope must stop growing before it can close
...
**Under freeze, this stage files into the frozen phase only `[blocker]` findings.** Every `[should-fix]` routes to `docs/FUTURE-WORK.md` (same shape as budget overflow, marked `deferred by phase freeze`) or, when it genuinely belongs to a successor phase's surface, to THAT phase's section marked not-pickable on the current branch per the cross-phase rule. The `[blocker]` exemption is the safety valve: a freeze must never be able to ship a broken release.

**Read the banner ONLY from the phase you are reporting, and never let `frozen` fall from `yes` to `no` on its own.** Two distinct failures, both observed in one real run. (a) The banner was looked up without scoping to the reported phase, so a `**FROZEN ...**` line still sitting under a CLOSED phase's `## Next up` subsection was read as if it governed the active one, and two iterations reported `frozen=yes` for a phase that had no banner at all. Match the banner to the subsection of the phase named in `phase=`; a banner under any other phase's subsection belongs to that phase and is invisible here. (b) Those same iterations then reported `frozen=no` from the third onward, which this contract does not permit: freeze is per-phase state that only a human lifts, normally by merging the phase. A stage that reports `frozen=yes` and later `frozen=no` for the same phase, with no human action between, has either misread the banner or silently dropped it. When your reading disagrees with the previous sample for the same phase, the banner is the authority and the disagreement is a finding about the earlier reading, never a licence to unfreeze. **Housekeeping that removes (a) at the source:** when a phase closes and its branch merges, its freeze banner has outlived its subject, so strike it in the same §5 edit that records the phase closed, and no later phase can inherit it.

**Record the freeze where every later stage reads it,** so it is state rather than a judgment each stage re-derives. On the review that first observes the condition, write `**FROZEN <utc-iso>: blockers only; should-fix routes to FUTURE-WORK.**` into that phase's `## Next up` subsection banner in `docs/TODO.md`, and name the transition in §6. Freeze is a property of the phase, not of one review: once the banner exists, later reviews and the dev cycle both honor it without recomputing, and only the human lifts it (normally by merging the phase and opening the next one).

**Freeze is not a bail.** The dev cycle keeps picking and closing the phase's remaining items exactly as before; what stops is the phase's growth on non-blockers. A frozen phase therefore drains monotonically and reaches the completion bail on its own.

## Prose findings in the handoff docs are §5 edits, never queue items

A finding whose ENTIRE remedy is text in one of the three files this stage already writes (a close note that overstates what was verified, a stale count, an acceptance clause the shipped commit made unreachable, an item description that contradicts a measurement) is applied in place in §5 and named in §6. It gets no `<phase>.<n>` ID, no `## Next up` entry, and no budget charge, because filing it as an item spends a whole cycle (pick, implement, test, review) to change a sentence in a file you have open right now. §0's tester-superseded-text exception is the same mechanism; this generalizes it to prose the review itself finds. Measured on the same run as the filing-budget figures: 19 of the active phase's 61 open items were of exactly this shape (corrections to prior items' close notes, docstring wording, comment accuracy), so about a third of the backlog was spent re-describing work rather than doing any.

**The boundary is which file the fix lands in, and it does not bend in the other direction.** Prose inside `docs/SPEC.md`, `docs/TODO.md` or `docs/FUTURE-WORK.md`: fix in place, always. Prose inside a TRACKED source file (a comment, a docstring, a README sentence): you may NOT fix it here, because that is the "no silent rewrites" rule in the operating principles and a docstring is code review's subject rather than its scratchpad. Judge it against the severity bar like anything else: a comment that would mislead the next engineer into a real mistake is a `[should-fix]` and consumes budget; a comment that is merely imprecise is below the bar and gets dropped, not parked.
 ...
 **Recovery-first health predicate (Phase 43/43.3).** Recover the orphaned cycle ONLY when all five signals hold — an incomplete-but-*healthy* cycle, i.e. the dev "just missed" its own commit:
 ...
 - **Dev-authored dirty tree** — the dirt is attributable to the dev's own run this iteration, shown by EITHER `docs/TODO.md`'s `## In flight` section carrying a live `- [` bullet, OR the dev skill's transcript for this iteration naming the dirty paths as work it performed (it reports editing or fixing them, or its own closing report describes the change as sitting in the working tree). Check the transcript whenever the bullet is absent rather than stopping at the bullet. The In-flight bullet is bookkeeping the dev writes at the START of its cycle, so a dev disorganized enough to skip its commit is disproportionately likely to have skipped the bullet too, and a predicate that keys on the bullet alone therefore heals the tidier failure while letting the worse one through, which inverts the order you want. Transcript attribution is also the stronger of the two signals on its merits: the bullet asserts only that some cycle started, while the transcript names the specific files and what was done to them, which is what the recovery commit actually needs. AND
 ...
 b. Read `docs/TODO.md`'s `## In flight` section (strip HTML comments). If it carries a live `- [` bullet, the **dev-authored dirty tree** signal holds; proceed to step (c). If it does not, do NOT skip yet: read this iteration's dev transcript from the run-log directory and check whether it claims the dirty paths as its own work. If it does, the signal holds on the transcript instead, and you proceed to step (c) with the transcript as the description source in steps 2 and 6. Only when NEITHER source attributes the dirt to the dev do you skip to step 3, treating the dirt as unrelated noise. If no dev transcript is readable at all (a harness that emitted none), the bullet is the only available source and its absence skips as before.
 c. **Both signals present (dirty tree + dev-authored attribution from step b); HEAD unchanged is established by the runner's routing.** Recovery path — note the **sanitize clean** gate (sub-step 4) runs BEFORE staging + commit so it is never the step immediately before the commit (Phase 43/D2 seam fix):
 ...
 2. Tests pass. Extract the scope + description from the In-flight line (format: `- [<skill> @ <utc>] <description>`). When step (b) attributed the dirt through the transcript instead, take the description from what the dev's own report says it changed, and use the dev skill's start time from the iteration manifest in place of the In-flight timestamp.
 ...
 6. Finalize `docs/TODO.md`: delete the In-flight bullet from step (b) when there was one; prepend to `## Just shipped`: `- <description> — by sst-dev-cycle at <utc>`, using the description and timestamp resolved in step 2.
...
4. **Tester findings (back-compat: review proceeds unchanged when files are absent).** Locate the chain run-log dir (most recent `.skill-runs/<*>/` under the current working directory, or the path the chain runner printed as `[log-dir] <path>`). **In a looped run, read the findings from THIS iteration's log subdir, not the run dir.** The run dir of a `--loop N` run is a single slot shared by every iteration, so a `tester-findings.json` sitting directly in it may belong to an EARLIER iteration; the iteration's own findings live beside the iteration's MANIFEST at `<run-dir>/iter_NN/`. Resolve the path the same way §2.10 resolves the MANIFEST: if the run dir contains `iter_NN/` subdirectories, read `<run-dir>/iter_NN/tester-findings.{json,md}` for the highest-numbered iteration (the one whose `MANIFEST.json` `git_sha_before` is an ancestor of HEAD — see §2.10; an exact `HEAD~1` match is NOT required, since a dev that took a preparatory commit starts from `HEAD~2` or earlier); only a flat, non-looped run puts the files directly in the run dir. Never read a run-dir copy without first checking for an `iter_NN/` subdir — and if both exist, the iteration copy wins and the run-dir copy is stale by construction. If the tester stage ran for this cycle, `tester-findings.json` and `tester-findings.md` are present at the resolved path. Read both when present; when absent, the tester was either pre-empted (`[skip-tester]`) or not yet inserted into the chain — in both cases the review proceeds exactly as before 41.3 with no gap flagged for tester absence.
 ...
 - A check with `status: pass` is a non-finding for §4 purposes, with ONE exception that is not a finding but a §5 EDIT: when a `pass` check's recommendation names a correction to the TEXT of an OPEN spec item (a hedge the tester has now measured, an acceptance clause the just-shipped commit made unreachable, a figure the tester confirmed or corrected), apply that correction to the item's own text in §5 alongside the new findings, and say so in the §6 report. Narrating it in §6 alone is not enough: the next cycle picks that item and reads its stale text, so a tester-superseded hedge sends it to re-measure what the tester already measured, and a superseded acceptance clause sends it to write a guard against a state that can no longer be reached. The tester is the only stage that measured the item live, and a `pass` verdict on the measurement does not make the correction optional. Mirror the same correction into the queue file (`docs/TODO.md`) when the item is also queued there — the dev reads the queue entry before the spec item. **The same exception reaches a close note this cycle just WROTE, not only an open item's text.** An item the dev closed this cycle carries a DONE/close note asserting how its acceptance was discharged, and a `pass` check can falsify that note's PROVENANCE while confirming the behaviour: the guard really does redden, just not in the state the note claims the reading was taken in. Scoped to open items alone, the rule skips this on the reasoning that no future cycle will pick a closed item; but a future cycle does read that note to decide whether the guard was verified against what shipped, and by then the tree it describes is gone, so a false provenance sentence there is unfalsifiable. So when a `pass` check's evidence contradicts a provenance claim in a note THIS cycle wrote, correct that sentence in §5 alongside the open-item corrections. **`Evidence` here means the check's own readings, not the tester's stated reason for taking them.** A tester that rebuilds an instrument rather than reuse the dev's narrates why it distrusts it; that is an independence choice, not a measurement. Establish the contradiction from something you can check yourself - for an instrument's provenance that is the dev transcript (did an edit to the measured module land after the instrument loaded it, was the run repeated after the commit) - and when it shows the instrument was byte-equivalent to what shipped, the reading stands: correct only the narrower claim the check does support, such as a route the note's evidence could not take, and leave the reading's attribution alone. Invalidating it on a premise the same transcript refutes writes exactly the unfalsifiable note this rule exists to prevent. Correcting it somewhere else does not discharge this: writing the true version into your own findings block while leaving the false sentence standing in the item's own note publishes both, and the item's note is the one a reader of that item reaches first.
...
**Locating the dev transcript is a mandatory first action of this axis — not a precondition to judge as unmet before looking.** List `.skill-runs/` and open the most recent dev transcript (`.skill-runs/*/iter_NN/00_<dev-skill>.txt` or `.skill-runs/*/00_<dev-skill>.txt`), plus its own `<i>_<dev-skill>.retry-<n>.txt` siblings whenever the dev's MANIFEST record carries `retry_count > 0`, each read WHOLE rather than tailed: the block precedes the first tool call, so it sits near the top of a long attempt. Every runner's sentinel capture reads ASSISTANT TEXT ONLY, and scans the FINAL attempt only; `sst-dev-cycle` §1 binds the dev to that same channel ("never through a tool call": a heredoc or an `echo` is the identical miss). So discharge on the CHANNEL, not on the bytes: a block that appeared as assistant-visible text in ANY attempt is the pick and DISCHARGES a `batch_pick_missing` / `picked_difficulty_emitted: false` MANIFEST flag as a capture artifact, here and in §2.10's re-band branch alike, while one that reaches the transcript only inside a tool ARGUMENT or a tool RESULT (in the prettified `.txt`, indented under a `-> <Tool>` line) CONFIRMS the flag: read the composition and rationale off it for this axis, and still report the dev-compliance miss. Bracket-parse the commit instead only when no attempt carries a block on either channel. The axis applies whenever some attempt used the batching protocol. If no dev transcript exists after the lookup, fall back to the `## Just shipped` top entries in `docs/TODO.md` as a proxy and note the fallback in §6. Never report this axis as "nothing to flag" without having opened the transcript or recorded the fallback.
...
**When a `[batch-pick]` cites a work-shape carve-out this review finds SATISFIED, the carve-out substitutes each EDGE it actually moves, and the no-op test applies PER EDGE rather than to the carve-out as a whole.** A ceiling above the tier's licences spending past it: band `oversized` against `max(tier ceiling, carve-out ceiling)`. A floor below the tier's is the case a ceiling-only reading gets wrong: when the carve-out MANDATES SOLO, `undersized` cannot fire at all, because its unbatched-queue condition is already defeated by the mandate (no sibling was batchable), so compare against the cited range and never report a pick that hit its declared budget as sitting below a floor it was forbidden to reach. Absent a solo mandate the tier's floor and published threshold govern `undersized`: a carve-out is never an obligation to spend the extra. A cited carve-out that is NOT satisfied moves no edge: band on the tier and report the unmet condition separately. Name both the compared band and the tier band in the §6 receipt. The machine line, when one fires, still carries `band=` for whichever band the comparison actually used.
...
This line is the supervisor's §3.5.1 extraction target and must appear as a standalone line in the transcript. If no batch-sizing finding fires this iter, do not emit the line -- and do NOT emit a tag-bearing substitute for it either. The literal `[batch-sizing]` token is the supervisor's grep key, so a no-fire receipt written as its own tagged transcript line registers as a phantom finding. The receipt's only home is the tag-free §6 `Batch-sizing:` clause. A "within band" claim is compliant ONLY when it states BOTH the computed `actual=` value AND the band edges it was compared against, AND `actual` falls numerically inside that band (or in a legal no-line gap: below the lower edge but at-or-above the undersize threshold). A receipt that quotes the actual but names no band, whose `actual=` lies outside the band it names, or that is purely qualitative, has skipped the comparison: re-run it and emit whatever the fire-rules actually produce. Eyeballing the diff's size instead of comparing the summed MANIFEST metric is a silent false-negative.
...
### 2.11 Queue-delta accounting

The filing budget and the freeze condition are per-review rules; whether they are WORKING is a cross-run question no single review can see. This axis emits the sample the supervisor aggregates (`sst-supervisor` §3.7), on the same handoff shape as §2.10's.

Emit a machine-parseable line to stdout at the end of §4 (or at the end of §3 when §4 does not fire), **exactly once, on EVERY iteration**:
...
**`phase_open` is RE-COUNTED from the spec file at write time, never carried forward from the previous iteration's line and never adjusted arithmetically from your own deltas.** It is the one field in the line that is a measurement of the world rather than a tally of your own actions, and it is the field the supervisor's flat-backlog trigger keys on, so a value inherited from the last sample makes a moving backlog look pinned and a stuck one look like it is being measured. Count it with the same command the phase-completion bail uses, after your §5 edits have landed, and expect it NOT to equal `previous - closed + filed + dev_filed`: items get parked, swept, re-homed to other phases, or closed by hand between iterations, so the arithmetic and the count legitimately disagree. When they do, the COUNT wins and the divergence is worth one clause in the §6 clause.

**Unlike `[batch-sizing]`, this line fires unconditionally,** including on a clean review (`filed=0 parked=0`) and including when the axis has nothing else to say. The supervisor reads it as a time series, so a missing sample is not "no change", it is a hole that makes a growing backlog look stable. Emit it as a standalone line with the tag leading. The literal `[queue-delta]` token is the supervisor's grep key, so do not quote it inside narrative prose, do not emit a second tagged line to explain the first, and do not reproduce a counterfactual version of it: both failure modes are documented against `[batch-sizing]` in `sst-supervisor` §3.5.1 and each cost that window real samples.

**The machine line is the SOLE handoff of a queue-delta signal; do NOT also route it through §4.** Its resolution is the supervisor's backlog-growth response (a filing-budget or freeze-threshold adjustment, in prose the supervisor governs), not autonomous dev-cycle work. So a queue-delta observation is never a spec item, never a `## Next up` entry, never a FUTURE-WORK line, and never counts toward §3's routed-finding total. Same reasoning as §2.10's identical rule: filing it injects skill-prose work into the dev's own pick queue and can provoke a single-iter threshold change that §3.7's cross-iter gating exists to prevent.
...
**"Findings" and "filed items" are different counts, and step 5 keys on neither alone.** The filing budget, the freeze rule, and the handoff-doc-prose rule all route real findings somewhere other than the spec, so a review can hold five findings and file zero queue items. Go to step 4 whenever you have at least one finding at the bar; go to step 5 and commit whenever step 4 wrote ANY of: a spec item, a `## Next up` entry, a `docs/FUTURE-WORK.md` line, an in-place prose correction, a freeze banner, or a third-tier sweep. Report clean (and still emit §2.11's line) only when nothing at the bar survived honest examination.
...
- Only `[blocker]` and `[should-fix]` severities go here; see the severity bar's "binds every writer" rule for why a third tier in a pick queue is a category error rather than a spare label, and for the sweep that moves any you find to `docs/FUTURE-WORK.md`. No nice-to-have / nitpick / cosmetic items: if you can't justify why it causes a real bug, security risk, or major confusion, don't file it.
- **The filing budget and the phase freeze gate this block, and they are applied HERE, once the findings list already exists.** Compute `closed`, enforce `filed_non_blocker <= max(1, closed)` capped at 3, honor the phase's freeze banner when one is set, and route everything above the line to `docs/FUTURE-WORK.md` with its evidence intact. Then emit §2.11's `[queue-delta]` line. Do not skip the arithmetic on the grounds that the findings all look important: that they are all real is the normal case, and it is exactly the condition the budget exists for.
...
> Reviewed commit `<sha>` (`<scope>: <summary>`). Checked all review axes (parity, correctness, coverage, docs, prod-verify, security, style, performance). No substantive findings at the blocker or should-fix bar. Spec unchanged. Tester: <green|skipped|degraded|red> (<n> checks: <by-status>). Batch-sizing: <actual=<n>k vs <band-compared> -> <outcome; see clause rules below>. Queue-delta: <closed=<n>, filed=<n> (<b> blocker), strengthened=<n>, parked=<n>, phase_open=<n>, freeze=<state>; dedup: <receipt>>.

**With findings:**

> Reviewed commit `<sha>` (`<scope>: <summary>`). Found <N> items: <B> blocker, <S> should-fix. Appended a "Review follow-ups" block under `<section>` in the spec and committed as `<review-sha>`. Highest-impact: <one-line description of the worst item>. Tester: <green|skipped|degraded|red> (<n> checks: <by-status>). Batch-sizing: <actual=<n>k vs <band-compared> -> <outcome; see clause rules below>. Queue-delta: <closed=<n>, filed=<n> (<b> blocker), strengthened=<n>, parked=<n>, phase_open=<n>, freeze=<state>; dedup: <receipt>>.
...
`Tester:` line rules: the label is the overall `verdict` (`green` / `degraded` / `red`), or `skipped` when that is the verdict or the findings file is absent. Break the `checks[]` count down BY STATUS (`9 checks: 7 pass, 2 needs-change, 0 fail`), never as a bare total: the label may legally disagree with the checks (§0 item 4 permits `green` over a residual `needs-change`), so a label-plus-total reports a residual-carrying tester as clean, the omission the disposal gate forbids. Absent files: `Tester: skipped (0 checks)`, so the supervisor tracks tester coverage across iterations.

`Batch-sizing:` clause rules: this clause is the §2.10 axis's mandatory receipt and appears in EVERY report, both forms. It is not a closed list of templates. State `actual=<n>k`, the band you compared against (tier `band=<lo>-<hi>k`, or `carve-out budget <lo>-<hi>k (cited and satisfied; tier band <lo>-<hi>k)` when a substitution stood, or `tier band <lo>-<hi>k (carve-out cited; substitution declined)` when it did not), and the outcome: `in-band, no line` when `actual` sits inside the compared band; `below band, above undersize threshold (<t>k), no line` when it sits below the lower edge but at-or-above the undersize threshold; `below undersize threshold (<t>k), unbatched-queue condition not met, no line` when the undersized conjunction fails on the queue side (repeat the exclusion reason in a few words, or say it was unavailable if `batch_pick_missing`); `line emitted: direction=<dir> actual=<n>k band=<lo>-<hi>k` when a finding fired; or `axis skipped: iter MANIFEST absent` (legal only after the lookup ran and found no file). Do not force a wording that asserts a false relation. A report with no `Batch-sizing:` clause means the axis was silently skipped.

`Queue-delta:` clause rules: mandatory in EVERY report, both forms, and it is the human-readable twin of §2.11's machine line rather than a substitute for it (emit both; the line is what the supervisor greps, the clause is what a reader audits). State the same counts (including `dev_filed`, or `?` when unresolved), the freeze state, and the dedup receipt. Two things the clause must carry that the machine line cannot: WHY anything was parked (`parked=2 (budget: closed=1)`, `parked=3 (freeze)`) and, when `filed` exceeded the cap, which exemption licensed it (`filed=4 (3 blocker, exempt)`). A `filed` above `max(1, closed)` with no blocker exemption named is a budget violation, so write the number honestly and say what happened rather than adjusting the count to look compliant: the supervisor's §3.7 audit recomputes `closed` from the commit anyway, and a clause that disagrees with the diff is a finding against this stage. `Found <N> items:` counts findings that survived the bar and landed somewhere durable -- new IDs plus budget/freeze parks to `docs/FUTURE-WORK.md` -- so `N = filed + parked`, and an all-parked review still takes the With-findings form, the only oversight a park gets. `strengthened` never enters it, being already in the queue.
