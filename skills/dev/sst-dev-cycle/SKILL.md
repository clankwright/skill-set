...
**The declaration pair is not a stopping point — do not end your turn on it.** The `[batch-pick]` + `[picked-difficulty]` blocks read like a finished deliverable: formatted, self-contained, addressed to the runner rather than to a caller awaiting an answer. That makes them a natural response boundary, and they are not one — at that instant the cycle has produced nothing. The next thing after `[picked-difficulty]` is §2's first tool call, in the SAME turn. A session whose final assistant message is the declaration pair has shipped no test, no implementation, and no commit, and has spent its entire iteration on the pick: the runner flags an incomplete cycle, its commit re-prompt cannot heal it (there is nothing in the working tree to commit), and the queue item returns untouched for the next invocation to re-research from scratch — the pick's research lives only in the discarded context. Observed: a cycle spent 28 turns researching a three-item batch across six files, emitted both sentinels, and ended with `stop_reason: end_turn` and `terminal_reason: completed` — a voluntary end-of-turn, not a turn-ceiling chop — having written no code, for a full-cycle cost and zero output. This guard is the mirror of the two below: those defend against the block being omitted, this one defends against it being emitted and then treated as the deliverable.
...
**Verification scope is set by the change's blast radius, not by the item's surface.** When the change edits a SHARED definition (a global style rule, a base class, a default parameter, a shared config key) rather than one call site, the item's own surface is the *least* likely place it breaks: that is the surface you designed it against and the one your new tests already cover. So before the gate counts as passed, enumerate what else the edited definition reaches (grep its consumers) and verify at least ONE consumer outside the item's own surface. Two traps make this non-optional. First, a change that NARROWS a shared definition can still WIDEN its precedence: adding a guard so the definition stops applying in one case can raise its override rank above the more specific per-consumer definitions that were previously winning, so the guard lands correctly where you were looking and silently takes over everywhere else. Second, a unit test that hand-models the resolution order for the two consumers you care about cannot go red on that: the model is structurally blind to consumers it does not enumerate, so a fully green suite is not evidence here; only a rendered or executed consumer is.

**A difference is not a pass, and a fabricated subject is not a consumer.** These are the two ways an outside-the-surface check reports success without earning it, and both read as a completed verification in the transcript. First, when you verify by COUNTERFACTUAL (measure the shipped state, rewrite the definition back to its pre-change form, measure again, assert the two differ), the delta proves only that the change took effect; it says nothing about whether the state you measured is the state the now-winning definition intends. So pair every counterfactual delta with an ABSOLUTE assertion: read what the winning definition declares and confirm the observed value equals it. Treat an observed value that is the property's initial or empty default (a transparent color, an unset string, a zero) as a FAILURE even when the delta is real, because that is the signature of a declaration which parsed but was dropped when it was computed, a reference to an undefined variable or token being the usual cause, and a pure difference test scores exactly that case as a successful hand-over. Second, when the real subject refuses to appear and you fabricate one to measure it (inject an element carrying the class, hand-build the record, seed the fixture), establish WHY it was absent before you substitute. "Absent because the current data does not produce it" and "absent because nothing in the product references it at all" are indistinguishable at the probe and opposite in meaning; one grep for the identifier across the source separates them. A fabricated subject can demonstrate a mechanism, never that a user can reach it, so a subject you could only fabricate must not be named as an affected or rescued consumer in a code comment, a spec entry, or a user-facing changelog line.

**A property of live data is a measurement, not a deduction.** When the change's design rests on what the running system actually holds (which keys a read endpoint returns for a real account, how large real stored values get, what an existing table's rows look like), reading the producer's source does not settle it, and a cycle that already has a live stack up is one query away from the answer. The recurring shape is a reader that layers runtime rows over a source-side defaults table: those declared defaults are a FLOOR on what the reader returns, never the set, so a guard whose inventory is derived from them is narrower than the reader's real output by exactly the rows nobody enumerated, and a size bound justified as having "orders of magnitude of headroom" is justified against a value that was never sampled. Query the real set ONCE, before you derive anything from it. Until you have, the property is unverified, and an unverified property must not be written into a code comment, a commit message, or a spec close note as though it held: those three artifacts outlive the cycle, each one is separate work for a reviewer to unwind, and the guard built on the same premise ships alongside them. A test cannot rescue this either; an assertion that iterates the source-side constant while its name claims a property of the reader's output is a tautology no code-change probe can see, because the substitution is in the assertion's subject rather than its predicate. Fixture at least one member the constant does not contain, or the claim is untested.
