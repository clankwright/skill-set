...
- **Output dir**: `<project>/data/short-video-generator/<series-id>/` for individual shot files (typically `.mp4`) and storyboard metadata (`storyboard.json`). `<series-id>` is either user-supplied or auto-generated as `<utc>_<short-uuid>`.
