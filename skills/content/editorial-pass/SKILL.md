...
name: editorial-pass
description: |
  Run a draft through a structured multi-pass editorial review — high-level scope check, citation verification (via fact-checker sub-skill), claim segmentation, fact-correction application, then a final clarity/conciseness pass. Produces a polished, publish-ready draft plus a summary of edits made and any remaining open questions. Designed to consume the output of iterative-writer or any human draft.
user-invocable: true
version: 1.0.0
...
A serious editor reads a draft in passes, each handling one concern. This skill encodes that procedure: scope first, then facts, then prose. The `fact-checker` sub-skill handles the verification work.

## Project contract

- **Output dir**: `<project>/data/editorial-pass/` for the edited draft. Files as `<utc>_<draft-slug>.md`.
- **Required skills**: `fact-checker` (called as a sub-skill).
- **Tools required**: harness's `Read`, `Skill` (for invoking fact-checker), `Write`. Web access only as needed for fact-checker's downstream search.
...
- **Cite the corrections.** When you fix a fact, name the source from the fact-checker that backs the correction. If the fact-checker came back unverified, mark the claim as `[unverified]` in the edited draft — don't quietly delete it.
...
Group related claims that share a search angle. Send the batch to the `fact-checker` sub-skill:
...
Wait for the fact-checker's report. It will return three buckets: verified / corrected / unverified.

## Pass 2: apply corrections

For each corrected claim: rewrite the sentence in the draft to use the fact-checker's corrected statement. Keep the surrounding prose; just swap the wrong fact for the right one. Add the source URL inline if the draft uses citations.
...
Save to `<project>/data/editorial-pass/<utc>_<slug>.md` with this structure:
...
## Edit summary
- Verified claims: <N>
- Corrections applied: <N> (from fact-checker report at <path>)
...
- **Never apply a "correction" without a fact-checker source.** If the fact-checker said unverified, the inline marker stays — it doesn't get rewritten in either direction.
...
- **Don't bypass fact-checker.** Even when the draft is short, every factual claim goes through the sub-skill. The whole point is the audit trail.
