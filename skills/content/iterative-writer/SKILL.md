...
description: Draft long-form content from a prompt and iteratively improve it through internal critique cycles. Each iteration calls the literary-critic skill, applies its feedback, and produces a better draft. Stops when the critic returns a "solid; minor polish only" verdict OR after a configurable max iterations (default 3). Final output is the polished draft, ready for an editorial-pass or direct publication.
user-invocable: true
version: 1.0.0
...
Draft → critique → revise → repeat. The skill drives the loop; `literary-critic` provides the feedback.

## Project contract

- **Output dir**: `<project>/data/iterative-writer/` for the final draft. Drafts as `<utc>_<topic-slug>.md`. Intermediate drafts can also be saved alongside as `<utc>_<topic-slug>_iter<N>.md` for traceability.
- **Required skills**: `literary-critic` (called as a sub-skill via the harness's `Skill` tool).
- **Tools required**: harness's skill invocation for the critic; `Write` for saving drafts. No web access — research belongs in `web-research`, called upstream of this skill.
...
Produce a complete first draft. Not an outline, not a sketch — a real draft of the requested length and form. Save to `<project>/data/iterative-writer/<utc>_<slug>_iter1.md`.

## 3. Critique loop

For each iteration `N` from 1 to `max_iterations` (default 3):

1. Invoke the `literary-critic` skill on the current draft. The critic returns a feedback report (path printed in its final line).
...
4. Otherwise, write iteration `N+1`. Apply every "Suggested next pass" item the critic listed, in the order given. Save to `<project>/data/iterative-writer/<utc>_<slug>_iter<N+1>.md`.
...
Copy the final iteration to `<project>/data/iterative-writer/<utc>_<slug>.md` (without the `_iterN` suffix). This is the canonical artifact for downstream skills (`editorial-pass`, `social-promoter`, etc.) to pick up via the project's standard latest-output discovery.
...
- **Never call yourself.** This skill is the loop driver; the critic is the only sub-skill it invokes. Don't recursively call iterative-writer.
...
- **Don't bake in self-promotion.** Even if the brief is for a brand piece, this skill writes the draft; brand-specific promotion text and links are added by `social-promoter` (or its proprietary counterpart) downstream.
