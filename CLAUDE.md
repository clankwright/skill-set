...
- **`/ssp-chain-driver --overnight --max-budget-usd 80`** (or higher) — unattended overnight drain on the same `skill-set-cycle` chain. Expands to `loop: 0` until failure / budget / Ctrl-C; randomized [5min, 30min] inter-iter delay keeps commit cadence human-shaped across many hours; the supervisor edits and commits framework skill improvements directly within the run, so later iterations consume them.
...
The proprietary cycle chain exists locally only (`.claude/` is gitignored as Claude Code runtime state). The transferable parent is `dev-cycle-with-review-looped` under `chains/` (stage order: `sst-dev-cycle → sst-tester → sst-dev-review`, with `sst-tester` inserted in Phase 41); overnight shape is the `--overnight` / `--preset overnight` CLI preset (Phase 42), not a separate YAML. Consuming projects build their own `<persona>-cycle` proprietary chain the same way.
