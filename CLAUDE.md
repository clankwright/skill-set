...
- **Outbound: pipe to `bin/notify-telegram.sh` with credentials in env.** Two ways: (a) `TELEGRAM_ENV_FILE=<env-path> bash bin/notify-telegram.sh` — the helper auto-sources the file when `TELEGRAM_BOT_TOKEN` is not already exported; (b) pre-source in a subshell, then pipe (`( set -a; . <env-path>; set +a; echo "..." | bash bin/notify-telegram.sh )`). Explicit shell env wins (the auto-source is skipped if `TELEGRAM_BOT_TOKEN` is already set), so a caller that exports the token deliberately is never overwritten by an `$TELEGRAM_ENV_FILE` value.
