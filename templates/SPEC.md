...
- **Two severities, and a bounded filing rate.** An open item is `[blocker]` or `[should-fix]`; there is no third tier, because a queue is a list of what a cycle does next and a polish item reaches the top the moment the real defects above it close. Polish goes to `FUTURE-WORK.md`. The writer skills also cap how many NEW items they may add per cycle against how many that cycle closed (see `sst-dev-review`'s filing-budget and phase-freeze sections), so that a phase's definition cannot outgrow its own closure rate; overflow parks in `FUTURE-WORK.md > Deferred review findings` with its evidence intact rather than being dropped.
