...
- [easy] [should-fix] 28.6 `bin/manager-bot.py:317` /feedback empty-body error missing `<project>` in usage string; user gets stale guidance and queues a token-less message the manager later refuses — review of fd86861
- [medium] [should-fix] 28.7 `bin/manager-bot.py:367-373` /status ignores project token; multi-persona deployments always get the same (newest) digest regardless of which persona was requested — review of fd86861
