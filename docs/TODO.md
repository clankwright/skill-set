...
- [medium] [should-fix] 18.10 `bin/drive-chain.py:main()` stale-recycle path (lines 833–843) wipes `WORKER_REFCOUNT_FILE`, dropping refcount slots held by concurrent drivers; under flock, check count > 0 before recycling and defer if non-zero — review of 4216269
- [easy] [should-fix] 18.9 `bin/drive-chain.py:_any_other_driver_using_persona` (lines ~520–547) docstring claims "Returns False conservatively (don't stop the worker)" but call site `if not _any_other_driver_using_persona(...): should_stop = True` does the opposite; align return-on-error with documented intent — review of 4216269
