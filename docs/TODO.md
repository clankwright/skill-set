...
- Phase 13 `Rate-limit pause-and-resume in skill-chain.py + schema + orchestrator hook` — reason: spec Phase 13; multi-iteration runs cross the 5h Anthropic quota mid-flight and currently abort the chain. Detect rate_limit_event status=exceeded (+ stderr/exit-code fallback for the case where the subprocess died before clean event emit), parse reset_time, sleep with jitter, re-invoke the killed skill in place. Three tiers (5h rolling, weekly/primary, burst/extra). Schema gains on-rate-limit + max-rate-limit-pause-seconds. Manifest records pauses for audit. Orchestrator Telegrams pause + resume.
