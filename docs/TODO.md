...
- [medium] Add a runner-level exponential-backoff retry for transient server errors (`API Error: 529 Overloaded` and other 5xx) in `bin/skill-chain.py` `run_skill_with_retry`, distinct from the rate-limit pause path: detect an `overload_signal` (result-frame `api_error_status` in {500,502,503,504,529} / `api_retry` system events / `API Error: 5\d\d` text), retry via `--resume` with exponential backoff + jitter capped at 10 runner-level retries (`--max-overload-retries`, default 10), then give up (`overload_retries_exhausted`); do NOT consume the `--max-pauses-per-session` budget or trigger the 8h pause. Reason: SPEC Phase 50; the 2026-06-23 cm-cycle run died at iteration 17 of 30 when a 529 in `ssp-cm-dev` aborted the loop with zero runner-level retries (the harness's own 10 internal `api_retry` attempts were already exhausted), per user request 2026-06-24.
- [easy] Gate the `wrote_tester_guidance` detection in `bin/skill-chain.py` `handle_event` on `name == "Write"` (or Write/Edit). It currently fires on ANY tool_use whose `file_path` ends in `tester-guidance.md`, so a dev that merely READS/Edits a stale guidance file trips the Phase 49 never-both gate and gets a legitimate `[skip-tester]` voided. Add a test for the Read-does-not-trip case. Reason: self-review of the Phase 49 gate false-positive vector, 2026-06-18.
- [medium] Add a structural guard so the cycle does not blind-ship e2e/test fixes whose failure only reproduces against a live stack. The `deleteFixtureCustomClaims` saga burned 4+ cycles shipping unverifiable hypotheses because the dev stage runs only Jest + `playwright --list` (parse-only), never the live import, so the tester falsifies each fix a cycle later. Options: a `[needs-live-verify]` item marker that routes to a gated tester / `/cm-deploy-test` pass instead of a normal dev pick, or an `sst-dev-cycle` rule that refuses to mark an e2e-only SPEC follow-up `[x]` without a live run. Reason: post-mortem of the SPEC 3.46 / HUMAN H3.13 blind-fix loop, 2026-06-18.
...
- [medium] Decide the `[batch-pick]` non-emission root cause. The dev stopped emitting the mandatory `[batch-pick]`/`[picked-difficulty]` block at the 2026-06-15 skill transition; the Phase 49 carve-out + runner `batch_pick_missing` flag stop it halting but the block is still never emitted, so batch-window refinement (`sst-supervisor` §3.5) and stuck-item detection (§3.6) run on fallbacks indefinitely. Either diagnose what changed in the dev prose/wrapper at 06-15 and restore emission, or formally accept the degradation and simplify §3.5/§3.6 so they do not depend on the block. Reason: masked-root-cause flagged in self-review, 2026-06-18.
