...
- [x] H43.1 [medium] **Durable fix for recurring `incomplete-cycle`: dev stops after the sanitize sub-skill, before the commit**
  ...
  Resolved: user chose option 2 (runner re-prompt) 2026-07-17; shipped as SPEC Phase 66 (`bin/skill-chain.py` resumes the dev session once with a commit directive on the incomplete-cycle signature; the sst-dev-review orphaned-cycle recovery remains the fallback net).
