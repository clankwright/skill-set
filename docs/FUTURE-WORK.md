...
- [hard] **Acceptance**: a `/ssp-chain-driver --loop 3+` run against a branch-per-phase fixture project whose active phase is fully `[x]` with no Next-up item scoped to it. Confirm: (a) sst-dev-cycle §0-7 emits `[no-work] phase <N> complete on <branch>; awaiting human branch setup for phase <N+1>` and the runner aborts the loop with MANIFEST `terminated_by: "no_work_bail"`; (b) §0-7a writes exactly ONE `docs/HUMAN.md ## Blocking` entry on the first iter and re-running the bail leaves the file byte-identical (idempotency — the gap that risks HUMAN.md/Telegram spam on a looped run); (c) `notify-human-md.sh` fires once on the fresh append and not on the idempotent skip; (d) a separate fixture where the same SPEC item is the picked primary across ≥3 trailing iters without its `[ ]`→`[x]` flip produces a sst-supervisor `[stuck-item]` finding plus one `docs/HUMAN.md ## High` decompose-or-remove entry plus one `manager-notes.md` observation (write-path (g)), all idempotent. Reason: 38.4 and 38.5 shipped as SKILL.md prose only (no unit test possible for LLM-driven skills); their acceptance criteria demand behavioral verification that only a live round-trip can supply.

### Phase 41.5+41.6 — ssp-cm-tester live chain-run acceptance

- [hard] **Acceptance**: a `cm-cycle` run (`bin/skill-chain.py --chain cm-cycle`, or `/ssp-cm-chain-driver`) on a CM feature branch whose dev iter touches a real front-end surface (e.g. an allocation or CSV-upload UI change) with a fresh `web/e2e/.auth/state.json` (<36h). Confirm: (a) `ssp-cm-dev` §7a writes a `tester-guidance.md` to the run-log dir naming the changed surfaces (or emits `[skip-tester]` on a backend-only iter and the runner never spawns `ssp-cm-tester`); (b) `ssp-cm-tester` starts the Flask :5003 + React :3000 stack, polls both ready, reuses the saved auth, runs the mapped `web/e2e/*.spec.js` with `--output` out of tree, and writes `tester-findings.{md,json}` to the run-log dir; (c) after the run `git status --porcelain` is byte-identical (zero in-tree artifacts) and no listener remains on :5003/:3000; (d) `ssp-cm-dev-review` surfaces the `Tester: <verdict> (<n> checks)` line and escalates any `fail`/`needs-change`. Reason: 41.5/41.6 shipped as SKILL.md + chain prose only (no portable unit test — the CM `.claude/` is gitignored runtime state); behavioral verification needs a live cm-cycle round-trip with Azure-AD auth.
