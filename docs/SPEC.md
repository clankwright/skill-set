...
- [ ] [easy] [should-fix] Phase 18 docs commit embedded framework-internal phase numbers in two transferables. sst-manager portion: fixed 2026-04-27 (dropped "Per the Phase 18 lifecycle policy," from Worker-lifecycle section; sanitize inline during sst-manager v1.2.0 cycle). Remaining: `skills/framework/sst-chain-driver/SKILL.md:151,158` — "Phase 18" and "the inbound-noise pattern Phase 18 exists to fix" still present; requires a sanitize pass + rephrase to drop phase numbering.
...
- [ ] [easy] [should-fix] `chains/dev-cycle-with-review-looped.yaml`, `chains/dev-cycle-overnight.yaml` (commit `4fceb04`) — transferable chain YAML defaults were tightened (`loop-delay-random` v1.1.0→v1.2.0 `[60,3600]`→`[60,600]`; v1.0.0→v1.1.0 `[300,7200]`→`[60,1800]`) but the commit shipped only the two YAML edits — no `docs/SPEC.md` closure note, no `docs/TODO.md > Just shipped` line. CLAUDE.md is explicit: "`SPEC.md`, `TODO.md`, and the code change ship in a **single commit**. Do not split them." This is a contract-drift event the framework's most-load-bearing rule guards against — leaving it unaddressed normalizes side-channel commits where transferable contract surfaces (chain YAMLs are first-class, schema-validated) move without a Just-shipped record. Proposed fix: add a backfill `Just shipped` line to `docs/TODO.md` summarizing the loop-delay-random tightening + version bumps + the proprietary-mirror motivation; ship it in the same commit as the next cycle's normal SPEC + TODO update so the rule isn't compounded twice in a row.
