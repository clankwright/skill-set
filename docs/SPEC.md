...
**Review follow-ups (open — schedule as the next `/sst-dev-cycle` cycle):**
- [ ] 42.8 [medium] [should-fix] `bin/skill-chain.py:2026-2063` — the `--profile` default-application block in `main()` (CLI-flag-wins-over-profile for each of chain/loop/max-budget-usd/max-cycles/telegram-env/label, plus the `explicit_loop` suppression of a profile `default-max-cycles`) has zero test coverage; only the loader `_load_profile_defaults` and the pure `_wrapper_halt_reason` helper are tested. 42.2's acceptance named "the budget/telegram/profile paths get tests," but the behavior-rich application path is uncovered, so a regression that lets a profile value clobber an explicit operator `--max-budget-usd`/`--max-cycles` cap (or that breaks the explicit-`--loop` suppression) would silently weaken the economic guardrail on profile-driven overnight runs and trip no test. Proposed fix: extract lines 2026-2063 into a pure helper `_apply_profile_defaults(args, profile, explicit_loop)` and add unit tests asserting (a) an explicit CLI flag wins over the profile value for each field, (b) an explicit `--loop` suppresses a profile `default-max-cycles`, (c) the profile fills only the fields the caller left unset.
