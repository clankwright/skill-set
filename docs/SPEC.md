...
- [ ] **Acceptance check.** Trigger a real five-hour rate-limit during a `loop: 3` run (likely by deliberately bursting a heavy skill run before the chain). Confirm: (a) detection fired, (b) sleep duration matched the reset time within ±60s, (c) the killed skill re-ran cleanly on resume, (d) the chain finished all three iterations, (e) MANIFEST records the pause with accurate timestamps, (f) Telegram fired both pause and resume notifications (now wired via `sst-chain-driver` / `bin/drive-chain.py`, formerly `sst-orchestrator` / `bin/orchestrate-chain.py`). User-driven; deferred until a real loop run crosses a quota boundary. Items (a)-(e) verified live on 2026-04-25 in `2026-04-25T13-36-00Z_skill-set-cycle/iter_03` (six_hour rejected, retry session `14f48ecd` resumed cleanly at the parsed wake_at, full chain completed). Item (f) still requires a chain-driver-wrapped run.
