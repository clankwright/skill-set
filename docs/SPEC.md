...
Closed review follow-ups: `[no-work]` sentinel false-positive gating via per-skill `git_sha_before` / `git_sha_after` comparison + `_no_work_bail_should_fire` helper; session-end Telegram parse failure resolved by defaulting `TELEGRAM_PARSE_MODE` to plain in `bin/drive-chain.py` `main()`.

### Phase 19: per-skill model-tier + item-difficulty-aware routing

Goal: maximize productive throughput within the user's existing Claude Max subscription by routing each chain iteration to the cheapest model that can handle the picked item, while preserving Opus for items that genuinely need it. Today the `claude-code` harness hardcodes `--model opus`, so every skill in every cycle burns Max quota at Opus rates regardless of whether the work needs frontier reasoning. The Max quota is denominated in compute-equivalents that roughly mirror API pricing: Sonnet ~5× cheaper per token than Opus, Haiku ~15× cheaper. Routing review-class work to Sonnet and mechanical work to Haiku roughly halves quota burn per iter (~2× more iters in the same Max window); aggressive Haiku use on bounded prose nits / queue drains gets to ~3-4×. No paid API spillover; this is purely sharper use of the existing subscription.

Two architectural layers, applied in `max()`:

1. **Per-skill `model-floor:`** in SKILL.md frontmatter. The lowest tier this skill is ever allowed to run on, regardless of item difficulty. Defaults by skill class:
   - **Opus**: `sst-supervisor`, `sst-sanitize-transferable` (framework-level routing / leak detection; floor never drops).
   - **Sonnet**: `sst-dev-cycle`, `sst-dev-review`, `sst-skill-router`, `sst-editorial-pass`, `sst-iterative-writer`, `sst-literary-critic` (substantial reasoning; structured tasks).
   - **Haiku**: `sst-translator`, `sst-fact-checker`, `sst-promote-skill-proposal`, `sst-output-selector`, `sst-llm-judge-ranker`, `sst-email-control-loop`, `sst-setup-telegram` (mechanical / well-bounded).
   Proprietary counterparts mirror their transferable's floor unless the proprietary has a stricter need.

2. **Per-item difficulty label** on every SPEC `[ ]` and TODO `Next up` entry. Required (validated). Three values:
   - `[easy]` → maps to Haiku tier (mechanical, well-bounded, no judgment-bleeding-edge).
   - `[medium]` → maps to Sonnet tier (substantial reasoning, multi-step, structured).
   - `[hard]` → maps to Opus tier (novel design, cross-file reasoning, architectural decisions, anything spec-closing on a complex phase).

Resolution rule (per skill, per iter): `effective_model = max(item.difficulty_tier, skill.model_floor)` where the `max` is over `{haiku < sonnet < opus}`. So `[easy]` item picked by `sst-supervisor` still runs Opus (floor wins); `[hard]` item picked by `sst-translator` runs Opus (item override wins); `[easy]` item picked by `sst-dev-cycle` runs Sonnet (floor wins, since dev-cycle's floor is Sonnet); `[medium]` item picked by `sst-translator` runs Sonnet (item override wins). Anti-fork rule still binds: skills with Opus floor cannot be downshifted regardless of item label.
...
- [ ] **Schema: `model-floor:` field on SKILL.md frontmatter.** Enum `opus | sonnet | haiku`; optional with default `opus` (backward-compat for skills that don't declare it; opt-in to lower their floor). Validator (`bin/validate-frontmatter.py`) surfaces missing/invalid values.
...
- [ ] **Runner: per-skill model resolution.** `Harness.build_command(skill_name, model=None)` accepts an optional model argument; defaults to the skill's `model-floor:`. `bin/skill-chain.py`'s `run_skill` reads SKILL.md frontmatter, resolves `effective_model = max(iter_difficulty_tier, skill.model_floor)`, passes to `build_command`. The hardcoded `--model opus` in `ClaudeCodeHarness.build_command` becomes `--model <effective_model>`.
...
- [ ] **Tag every transferable + proprietary SKILL.md with `model-floor:`.** Per the floors listed in the preamble. One commit; sanitize-transferable judgment pass on each transferable.
...
- [ ] **Acceptance**: a `/skill-set-chain-driver --loop 3` run with mixed-difficulty items (at least one `[easy]`, one `[medium]`, one `[hard]` across the three iters). Confirm: (a) per-iter MANIFEST records `difficulty` field; (b) per-skill `model_usage` block reflects the resolved model (Opus for hard items + supervisor; Sonnet for medium + reviews; Haiku where applicable); (c) cumulative quota burn per iter is ~50% of an all-Opus baseline run on the same items (target: ≥40% reduction); (d) supervisor still runs Opus regardless of item difficulty (floor invariant); (e) chain-driver session-end Telegram body reports per-iter difficulty + model breakdown.
