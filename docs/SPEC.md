...
- [ ] [should-fix] `docs/SPEC.md:321` (Phase 19 item #7 `sst-dev-cycle` updates): the cycle commit e48f273 actually shipped this item end-to-end (sentinel emit + don't-downgrade-or-upgrade prose at `skills/dev/sst-dev-cycle/SKILL.md:76-90`, plus `model-floor: sonnet` + `effort-floor: high` frontmatter at lines 4-5), but the item remains `[ ]` while item #3's closure paragraph at line 317 absorbed the dev-cycle change description verbatim. Net effect: future cycles reading the SPEC may re-pick item #7 as still-open work. Item #5 (Runner per-skill resolution) and #6 (Runner pre-parse + sentinel capture) correctly remain `[ ]` since the runner was not touched this cycle; only #7 over-claims as open. Proposed fix: flip item #7 `[ ]` → `[x]` with a brief closure paragraph noting "shipped bundled with item #3 in commit e48f273; the §1 `Difficulty label & sentinel emit` step lives at `skills/dev/sst-dev-cycle/SKILL.md:76-90`, and the skill's `model-floor: sonnet` + `effort-floor: high` frontmatter matches the Phase 19 review-class floor table." Trim the over-claim from item #3's closure paragraph if doing so cleanly (optional polish).
