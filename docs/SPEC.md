...
- [ ] 44.1 [hard] **Standalone mode in `sst-tester`.** In `skills/framework/sst-tester/SKILL.md`, add the user-invocable standalone mode triggered by `--phase <id>` and/or `--todos <ref...>` (distinct from the in-chain mode): resolve target -> enumerate the UI/UX surfaces those closed items introduced -> iterate, exercising each (run the mapped e2e spec(s) + exploratory checks), accumulating findings. Document the arg surface, mode dispatch (D1), scope resolution (D2), iterate-all/collect-all (D3), and the standalone artifact/findings location (D4). Keep the in-chain mode unchanged; standalone stays read-only / out-of-tree / full-teardown. Acceptance: SKILL.md documents both modes + `--phase`/`--todos` + scope resolution + the read-only/out-of-tree/teardown guarantees; `/sst-sanitize-transferable` must-fix 0; version bumped.
- [ ] 44.2 [medium] **Phase/todo scope resolution + tests.** Specify and test how the surface set is derived: `--phase <id>` -> all `[x]` items under `### Phase <id>` touching a front-end path; `--todos <ref...>` -> matching `## Just shipped` entries (by id or text); define how a UI surface is identified from a closed item (its changed files / named surface). Acceptance: documented resolution with an example for each input; a `tests/` test derives the surface set from a sample phase + a sample todo list.
- [ ] 44.3 [medium] **Standalone mode in `ssp-cm-tester` + CM phase->spec map.** Mirror 44.1 in the CM wrapper: expose the `--phase`/`--todos` standalone mode with CM's phase->`web/e2e/*.spec.js` map (e.g. Phase 3 custom-claims -> custom-claims.spec.js + submitted-claims.spec.js + the report specs), local-stack start/stop, auth reuse. Acceptance: `ssp-cm-tester/SKILL.md` documents the standalone invocation + the CM phase->spec map; `bin/check-ssp-sync.py` clean; `/ssp-cm-tester --phase 3` documented.
- [ ] 44.4 [easy] **Docs + invocation.** Document the terminal invocation (`/sst-tester --phase <id>` / `/ssp-cm-tester --phase 3`, and via the unified runner once Phase 42 lands) in `README.md` + `CLAUDE.md`. Acceptance: README/CLAUDE.md show the standalone invocation; full `tests/` suite green; `bin/validate-frontmatter.py` clean.

### Phase 45: tester fix-authoring authority (queue concrete fixes as TODO items)

**Context.** The tester (Phases 41/44) only reports findings (`tester-findings.{md,json}`) for the reviewer to triage into spec/TODO follow-ups; it has no authority to queue a fix itself. When the tester can pinpoint EXACTLY what needs fixing — a concrete file:line/symbol plus a specific change it is confident resolves the issue — routing through the reviewer adds latency and risks the finding being deprioritized or lost. This phase grants the tester BOUNDED authority to author a concrete `[fix]` TODO `## Next up` item directly for such cases, while vague or judgment-heavy findings still flow to the reviewer.

**Design decisions:**
- D1 — Bounded authority (the confidence bar): the tester MAY append a TODO `[fix]` item ONLY when the finding names the exact file:line (or symbol) AND a specific proposed change it is confident resolves the issue ("I know exactly what to change"). Vague, uncertain, or design-judgment findings stay findings-only and flow to the reviewer (Phase 41.3).
- D2 — Item format: the authored item follows the Next-up format + the Phase 38.1 bounded-item rule (concrete target + falsifiable acceptance), origin-tagged (e.g. `[fix] <phase>.<n> <file:line> — <fix>. — filed by sst-tester from <run>`), so the dev cycle picks it like any queued item.
- D3 — Never commits / never edits source (Phase 41 D5 unchanged): the tester only APPENDS the item to the working-tree `docs/TODO.md` (a handoff doc, not code); in-chain the downstream reviewer commits docs as normal, in standalone mode the item is written + surfaced in the terminal summary for the human/next cycle to commit. The tester runs no `git commit` and edits no repo source.
- D4 — No double-filing: the tester files ONLY exact fixes; the reviewer (41.3), seeing a tester-authored `[fix]` item already queued for the same `change_ref`/file:line, does not re-file it.

- [ ] 45.1 [hard] **Grant `sst-tester` bounded fix-authoring authority.** In `skills/framework/sst-tester/SKILL.md`, add: when a finding is concrete + actionable (exact file:line/symbol + a specific confident fix), append a `[fix]` `## Next up` TODO item (bounded-item format, origin-tagged) in addition to recording the finding; vague/uncertain findings stay findings-only for the reviewer. Document the confidence bar (D1), item format (D2), the never-commit/never-edit-source constraint (D3), and the reviewer dedupe contract (D4). Acceptance: SKILL.md documents the fix-authoring authority + the exact confidence bar + format + that it never commits or edits source; `/sst-sanitize-transferable` must-fix 0; version bumped.
- [ ] 45.2 [medium] **Reviewer dedupe of tester-authored fixes.** In `skills/dev/sst-dev-review/SKILL.md`, when consuming tester findings (41.3), skip re-filing any finding the tester already queued as a `[fix]` TODO item (match by `change_ref` / file:line). Acceptance: the inputs/consumption section documents the dedupe; a `tests/` test shows a tester-`[fix]`-queued finding is not re-filed by the reviewer; version bumped.
- [ ] 45.3 [medium] **Mirror in `ssp-cm-tester`.** In `ssp-cm-tester/SKILL.md`, expose the same bounded fix-authoring authority writing to CM's `docs/TODO.md` (gitignored runtime doc; tester appends, never commits). Acceptance: `ssp-cm-tester/SKILL.md` documents the fix-authoring authority + the CM TODO path + the confidence bar; `bin/check-ssp-sync.py` clean.
- [ ] 45.4 [medium] **Tests.** Add tests: a concrete finding -> tester appends a well-formed `[fix]` Next-up item that passes `validate_spec_item_quality`; a vague finding -> findings-only (no TODO item); reviewer dedupe. Acceptance: full `tests/` suite green; `bin/validate-frontmatter.py` clean; tests cover the confidence gate both ways.
