...
### Phase 43: stop the dev cycle halting before commit (sanitize-seam fix + recovery-first reviewer)

**Context.** The autonomous dev cycle (and its `sst-dev-review §0.2` recovery) repeatedly finish the work + tests + sanitize but exit WITHOUT committing, leaving a dirty tree and tripping the Phase 36 incomplete-cycle guard (`terminated_by: contract_violation`), which aborts the loop. Root cause, proven across three 2026-06-15 transcripts (24, 43, 24 of 150 turns, every one ending `(success)` — not `error_max_turns`, not an error): the cycle invokes `/sst-sanitize-transferable` as the LAST step before commit, and the agent treats that sub-skill's clean "must-fix: 0" return as task-completion, stopping its turn before the SPEC-flip / `git commit` + push. The recovery path has the identical defect (sanitize immediately before its commit), so the designed mitigation fails the same way. A prose anchor (`sst-dev-cycle` v1.7.1) was shipped but is the same class of fix. This phase fixes it two ways: (1) close the seam so the cycle reaches its own commit, and (2) make the reviewer commit a healthy-but-uncommitted cycle as its first action and stop the runner killing a loop that recovery can heal.

**Design decisions:**
- D1 — Seam fix: the sanitize gate must NOT be the last `/skill` sub-invocation before commit. `git commit` + push must be the skill's final action with NO `/skill` sub-invocation (sanitize or otherwise) between the test-green point and the commit. Mechanism is the implementer's choice — run the sanitize scan earlier (right after the transferable edit, before tests), OR inline the must-fix judgment against the shared `templates/sanitization-guidance.md` rubric instead of delegating to the sub-skill — but the binding constraint is "no sub-skill return sits between work-done and commit." Applied to both `sst-dev-cycle` §5 and `sst-dev-review §0.2`.
- D2 — Recovery-first reviewer: `sst-dev-review` checks, as the FIRST action of its turn, for an incomplete-but-healthy dev cycle (dirty tree + an In-flight TODO line + HEAD unchanged + tests green + sanitize clean = the dev "just missed" the commit). If found, it commits the dev's work immediately (the recovery commit) BEFORE its review pass, then reviews the now-committed change. A missed commit is healed at the top of the next stage rather than after a second sanitize-gated commit attempt.
- D3 — Drop the kill-on-miss when recovery succeeds: the runner's `contract_violation` (incomplete-cycle) abort fires ONLY if HEAD is still unchanged AFTER the review/recovery follower has run. A cycle the reviewer recovers (HEAD advanced during review) is NOT a contract violation and the loop continues. The kill remains only for a genuinely unrecoverable miss (no follower, or follower also left HEAD unchanged).

- [ ] 43.1 [hard] **Close the sanitize→commit seam in `sst-dev-cycle`.** In `skills/dev/sst-dev-cycle/SKILL.md` §5, restructure so the transferable sanitize gate is not the last `/skill` invocation before commit: either run the scan right after the transferable edit (before tests) or inline the must-fix judgment against `templates/sanitization-guidance.md`; ensure `git commit` + push is the final action with no `/skill` call between test-green and commit. Acceptance: §5 has no `/sst-sanitize-transferable` sub-invocation as the step immediately before commit; the commit is documented as the skill's final action; `/sst-sanitize-transferable` clean on `sst-dev-cycle`; version bumped.
- [ ] 43.2 [hard] **Close the same seam in `sst-dev-review §0.2` recovery.** Apply the identical restructure in `skills/dev/sst-dev-review/SKILL.md` §0.2 so its recovery commit is not preceded by a sanitize sub-skill invocation. Acceptance: §0.2 recovery has no sanitize `/skill` call immediately before its commit; commit is the final action; `/sst-sanitize-transferable` clean; version bumped.
- [ ] 43.3 [hard] **Recovery-first commit in `sst-dev-review`.** At the very start of its turn, before the review pass, `sst-dev-review` detects an incomplete-but-healthy dev cycle (dirty tree + In-flight line + HEAD unchanged + tests green + sanitize clean) and commits the dev's work then (the recovery commit), then reviews the now-committed change. Document the health predicate and the order (recover, then review). Acceptance: §0 documents the recovery-first step with the exact health predicate; a test/fixture shows an orphaned-but-healthy cycle is committed at the start of review; `/sst-sanitize-transferable` clean; version bumped.
- [ ] 43.4 [medium] **Relax the runner's `contract_violation` kill.** In `bin/skill-chain.py`, the incomplete-cycle abort fires only when HEAD is STILL unchanged after the review/recovery follower has run (a reviewer-recovered cycle continues the loop); keep the abort when there is no follower or the follower also left HEAD unchanged. Acceptance: a unit test where dev exits dirty + review commits → loop continues (no `contract_violation`); where dev exits dirty + no follower (or follower also fails to commit) → loop aborts as today; existing incomplete-cycle tests updated.
