...
- [x] **`bin/drive-chain.py` + chain driver implementation.** Helpers landed: `_persona_from_env_file` (derives persona from a `<persona>-telegram.env` path; returns None when the path doesn't follow convention so the caller skips management rather than guessing), `_tmux_session_exists` (graceful `FileNotFoundError`/timeout fallthrough so a host without tmux just produces a no-op probe), `_read_live_pid` (validates the PID is alive via `os.kill(pid, 0)`; stale-PID-file case returns None so a fresh start happens; `PermissionError` counts as live since the process exists but is owned by another user), `_probe_worker` (probes persona-suffixed tmux session, then legacy `manager-bot` tmux name from README's always-on pattern, then `~/.claude/state/manager-bot.pid` with liveness check), `_start_worker` (acquires `fcntl.flock(LOCK_EX | LOCK_NB)` on `~/.claude/state/manager-bot.pid.lock`, re-probes inside the lock to close the TOCTOU window, spawns `tmux new-session -d -s <persona>-bot "TELEGRAM_ENV_FILE=<env> python3 bin/manager-bot.py"`, captures the pane PID via `tmux list-panes -F '#{pane_pid}'` and writes it to the PID file; returns None when raced or tmux unavailable so the chain runs without an inbound worker rather than crashing), `_stop_worker` (idempotent `tmux kill-session` + PID-file unlink). `main()` start-of-session block: skips entirely when `--no-telegram` OR `--telegram-env` is unset; emits a stderr line for each branch (started / pre-existing / convention-mismatch / start-failed) so the operator sees what happened. End-of-session block runs after `proc.wait()`, before the final telegram body, gated on `worker_started_by_us`. Smoke-tested in-process: persona derivation from `skill-set-telegram.env` → `skill-set`, `sdrai-telegram.env` → `sdrai`, `foo.env` → None, `None` → None; tmux probe on a definitely-absent session name returns False without raising; PID file read on missing path → None, malformed integer → None, dead PID 999999 → None, live PID 1 → 1; `_probe_worker('bogus-persona-bot')` returns None on a host with no worker. Real-world acceptance check (item 5 below) deferred until the next chain-driver invocation since it requires a live Telegram round-trip.
...
- [x] [should-fix] `bin/skill-chain.py:499-503` (`handle_event` no-work sentinel scan) + `bin/skill-chain.py:1051-1058` (`run_iteration` bail) — sentinel detection is first-match-wins on any assistant-text line beginning with `[no-work]`, with the bail unconditionally skipping review/supervisor and aborting the loop on a clean exit. If a dev skill prints the sentinel mid-reasoning ("Here's what I'd emit:\n\n[no-work] queue empty\n\n...") and then proceeds to commit real work, the runner skips review/supervisor for a real commit AND aborts the loop. The cycle's smoke tests (per commit `16e67a1` body) covered "rejects inline mentions" but not "sentinel emitted on its own line as part of reasoning, then skill keeps working." Proposed fix: gate the bail on `git_sha_before == git_sha_after` for the dev skill (if a commit landed, the sentinel was a false alarm); or only honor a sentinel match in the LAST assistant-text block before exit, not first-match-wins.
  ...
  Fix landed: per-skill `git_sha_before` / `git_sha_after` sampled around `run_skill_with_retry`, and the bail is gated through a new `_no_work_bail_should_fire(record, sha_before, sha_after)` helper. When both SHAs are known and differ (real commit shipped), the bail is suppressed, the sentinel reason is moved onto `record["no_work_bail_suppressed"]` for visibility, and review/supervisor proceed normally on the commit. When either SHA is unavailable (non-git harness, `git rev-parse` failure), the bail still fires (the dev skill remains the authority on its own steady state). Helper exercised against 9 unit cases covering each branch.
...
- [ ] [should-fix] `bin/drive-chain.py:473-518` (start-of-session probe + start) and `bin/drive-chain.py:736-744` (session-end `_stop_worker`) (added in commit `97a6a51`): simultaneous chain-driver runs share a single worker process. Only the driver that started it has `worker_started_by_us=True` and kills the tmux session at session-end, while concurrent drivers adopt the worker via the `existing` branch with `worker_started_by_us=False` and rely on it for inbound. When the starter finishes first, `_stop_worker` kills the tmux session out from under any concurrent driver, dropping `/pause` `/resume` `/status` coverage for the remainder of those sessions. Item 5d's "two simultaneous chain-driver runs" acceptance criterion ("only one tries to start the worker (flock)") only verifies start-side flock behavior; cleanup overlap is unguarded. Proposed fix: refcount in `~/.claude/state/manager-bot.pid.lock` (incr at start-or-adopt, decr at session-end, only `_stop_worker` when count reaches zero); or at session-end probe for any other live `drive-chain.py` process bound to the same `--telegram-env` before killing.

- [x] [should-fix] `bin/drive-chain.py:461-475` (Telegram env merge in `main()`) + `bin/drive-chain.py:754-763` (session-end body): the chain driver's session-end body interpolates raw run-dir paths, ISO timestamps, exit codes, and commit subjects into a Telegram message that is sent under `notify-telegram.sh`'s default `Markdown` parse mode. Caught on the `2026-04-26T03-07-43Z_skill-set-cycle` run (3/3 iters clean, $25.17, exit 0): session-start, iter-close, and rate-limit bodies parsed fine, but the session-end body failed with Telegram API `Bad Request: can't parse entities: Can't find end of the entity starting at byte offset 242` (an unmatched `_` in the run-dir path parsed as an unterminated italic entity; `notify-telegram.sh` exited 1, the chain driver logged the soft-fail to stderr and continued). Soft fail (chain finished regardless), but the user lost the phone-side wrap-up message on every successful run, which is the whole point of the Telegram stream.

  Fix landed: the chain driver now calls `tg_env.setdefault("TELEGRAM_PARSE_MODE", "")` after the env-file and shell-environment merge in `main()`, so plain text is the default for every chain-driver body (session-start, iter-close, rate-limit pause/resume, supervisor escalation, session-end). Explicit user config still wins via `setdefault`: an env-file `TELEGRAM_PARSE_MODE=Markdown` or a shell-set `TELEGRAM_PARSE_MODE=HTML` are both preserved unchanged. This catches a broader class than the originally-suggested option (b) "touch only the session-end branch": iter-close commit subjects with `_` in kebab-cased identifiers, supervisor escalation paths with `[`, and rate-limit pause body skill names also stop being entity-parsed. Chain-driver bodies are status reports interpolating raw filesystem and shell-state values, not formatted prose; plain text is the right semantic default. Smoke-tested 6 env-merge precedence cases (no config / env-file Markdown / shell Markdown / env-file HTML / env-file empty / env-file beats shell) and 2 payload-builder cases (empty mode omits `parse_mode` from the API JSON, explicit mode is preserved); all pass.
