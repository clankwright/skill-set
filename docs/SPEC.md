...
- [ ] [should-fix] `bin/orchestrate-chain.py:297` — `looping = (args.loop is not None and args.loop != 1)` only inspects the `--loop` CLI override; it does not consult the chain YAML's `loop:` field. When the orchestrator is invoked against the just-shipped `chains/dev-cycle-with-review-looped.yaml` without re-passing `--loop 3` on the CLI (the canonical no-override invocation, since the YAML already declares `loop: 3`), `bin/skill-chain.py` resolves `loop_count = 3` and writes `iter_NN/MANIFEST.json` plus `===== iteration N/3 =====` banners, while `_finalize_iteration` (line 353) reads the flat `log_dir/MANIFEST.json` (which only exists at session-end). Per-iter Telegram bodies fall to the "MANIFEST.json not found" branch (line 360-364), `cumulative_cost_usd` stays at $0.0000, and the `--max-budget-usd` halt (line 396) never fires. Proposed fix: derive `looping` from the iter-banner regex (the banner only fires when the runner resolved `loop_count != 1`, so set `looping = True` on first match), or resolve the chain YAML's `loop:` field at startup the same way `bin/skill-chain.py` does.
