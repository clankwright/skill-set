...
- [ ] **`sst-dev-cycle` §0 empty-queue bail.** (group with empty-queue-bail) Add a pre-flight check: if `TODO.md > Next up` is empty AND every `- [ ]` in `docs/SPEC.md` is `[x]` AND no user-provided task is in the prompt, exit 0 cleanly with a recognized no-work sentinel printed on stdout (sentinel format defined by item 2 below). The dev skill MUST NOT pick a just-shipped item, MUST NOT invent speculative work, MUST NOT scope-creep on existing skills. The bail is the correct response, not a defect. Proprietary `<project>-dev-cycle` skills inherit the behavior automatically via `transferable:`; no per-project change required. Single skill prose addition (~30 LoC); version bump on `sst-dev-cycle` 1.1.0 → 1.2.0 (added behavior).

- [ ] **Chain runner recognizes the `[no work]` exit signal.** (group with empty-queue-bail) When any skill in a chain prints a recognized no-work sentinel on stdout (proposed format: `[no-work] <one-line reason>`) before exiting clean, `bin/skill-chain.py` aborts the current iteration's remaining skills (review, supervisor) AND aborts the loop entirely. Saves the per-iter overhead of running downstream skills against an empty commit. The sentinel is a chain-runner contract; sentinel format documented in `templates/SPEC.md` so any consuming project's dev skill can opt into the bail. Manifest records the empty-bail as `iter_manifest["no_work_bail"] = {"skill": "<name>", "reason": "<sentinel-line>"}` so consumers can distinguish a clean steady-state exit from a real failure. Top-level `manifest["loop"]` gains a `terminated_by: "no_work_bail"` field when the loop ends this way (vs `"max_cycles"` / `"max_budget"` / `"failure"`). The chain driver's session-end Telegram body reads "no-work bail" not "max-cycles reached" when this fires.

- [ ] **Documentation in README.md + CLAUDE.md.** "Loop mode" subsection extended: empty-queue bail is the canonical clean stop, not an error or edge case. "Chains shipped here" guidance for `dev-cycle-overnight` updated to note that the chain auto-stops on empty work even without a `--max-cycles` cap, so unattended overnight drains terminate cleanly when the queue is exhausted instead of looping on speculative work. CLAUDE.md "Choosing a chain" section updated to remove the standing "don't invoke when Next up is empty" caveat: the framework now handles it.
