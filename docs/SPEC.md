...
**One small change to `bin/skill-chain.py`**: the harness now spawns every skill with `--permission-mode bypassPermissions` instead of `--dangerously-skip-permissions`. The two are not equivalent in practice: only `bypassPermissions` has the documented carveout for `.claude/skills/**`, `.claude/commands/**`, and `.claude/agents/**` (writes there proceed without a prompt because Claude routinely writes there). `--dangerously-skip-permissions` empirically still prompts on `.claude/skills/` writes despite its "bypass all permission checks" help text, which breaks the direct-overwrite path. The supervisor reads `auto-promote` from the chain YAML (via `MANIFEST.chain_definition`) and routes accordingly. Rollback of an unwanted direct overwrite is `git checkout <path>/SKILL.md`.
