...
- [ ] [should-fix] `bin/skill-chain.py:186` — `_compute_rate_limit_sleep`'s `retry_after_seconds` branch reads the harness value via `float(signal["retry_after_seconds"]) + random.randint(*RATE_LIMIT_JITTER_RANGE)` with no `max(0.0, ...)` clamp. The parallel `reset_time` branch on line 183 does have `max(0.0, reset_epoch - now)`. A harness payload with a negative or stale `retry_after_seconds` (e.g. `-30` from a clock-skew or schema-shape mismatch across versions) yields a negative `sleep_seconds`, then `time.sleep(<negative>)` raises `ValueError: sleep length must be non-negative` from inside `run_skill_with_retry`. The exception propagates up uncaught and aborts the chain ungracefully: exactly the failure mode Phase 13 was meant to prevent (the runner crashes inside the pause-and-resume safety net rather than retrying the killed skill). Proposed fix: change line 186 to `sleep_seconds = max(0.0, float(signal["retry_after_seconds"])) + random.randint(*RATE_LIMIT_JITTER_RANGE)` to mirror the reset-time clamp; add a unit test covering negative + zero `retry_after_seconds` to the helper-test set already cited in the cycle commit message.
