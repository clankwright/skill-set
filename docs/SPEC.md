...
- [ ] **Orchestrator hook (Phase 12 #5).** `sst-orchestrator` / `bin/orchestrate-chain.py` recognizes `rate_limit_pauses` events and Telegrams: "Chain paused at <UTC>, resuming at <UTC>; reason: <type>; iteration <N>/<total>." A second Telegram fires on resume. Deferred until the orchestrator itself lands (Phase 12 tail item).

- [ ] **Acceptance check.** Trigger a real five-hour rate-limit during a `loop: 3` run (likely by deliberately bursting a heavy skill run before the chain). Confirm: (a) detection fired, (b) sleep duration matched the reset time within ±60s, (c) the killed skill re-ran cleanly on resume, (d) the chain finished all three iterations, (e) MANIFEST records the pause with accurate timestamps, (f) Telegram fired both pause and resume notifications. User-driven; deferred until a real loop run crosses a quota boundary.
...
- [ ] [should-fix] `bin/skill-chain.py:681-684` — `run_skill_with_retry` only threads the text-extracted `rate_limit_text_reset` into `eff_signal["reset_time"]` when `signal is None` (the stderr-only-death path). In the actual live failure mode this cycle was built to fix (structured `rate_limit_event` with `status: "rejected"` AND stderr `You're out of extra usage · resets 7:50pm (Asia/Tokyo)`), `signal` is not None, so the new `RATE_LIMIT_RESET_RE` extraction is ignored on the exact path the forensics describe. Whether the runner sleeps to the right wake time then depends entirely on the captured `rejected` payload happening to carry `reset_time` under one of the four aliased field names checked at `bin/skill-chain.py:502-505` (`resetsAt`/`reset_time`/`resetTime`/`resets_at`); a payload using a different field name yields `eff_signal["reset_time"] = None` and `_compute_rate_limit_sleep` falls through `reset_time → retry_after → 300s exponential backoff` — the very regression Phase 13's spec close item names. The cycle's own "Live-failure follow-ups" subsection (line 286) documents this conditional only for the stderr-only case, leaving the joint-fire case unaddressed. Proposed fix: change the merge condition from `if signal is None:` to `if not eff_signal.get("reset_time"):` so the text-extracted wall-clock fills in whenever the structured signal lacks a parseable reset, regardless of whether the structured signal itself fired. Update the manifest pause-record `source` field at line 711 to reflect mixed provenance (e.g. `"rate_limit_event+text_reset"`) so post-hoc audit can tell which branch supplied the wake time. Add an inline scenario covering signal-with-no-reset-time + text-fallback-reset to the verification set.
