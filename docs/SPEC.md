...
- [ ] Supervisor over-expands patches. First Phase 11 sdrai-cycle run articulated 2 findings but wrote a patch with 3 changes (the third — "Public landing surface" bullet — was orphan scope creep not grounded in the transcript). Tighten the supervisor's §3 wording to require every proposed line-level change to cite the transcript line(s) that motivate it, and reject drafts where the change set exceeds the finding count without explicit rationale. (The `proposed_line ↔ transcript_line` mapping is already implicit in the "Cited transcript lines" block; make it a hard gate instead of a courtesy.)
